...
using fiorch::services::alpha_arena::FillEvent;
using fiorch::services::alpha_arena::FundingEvent;
using fiorch::services::alpha_arena::IExchangeVenue;
using fiorch::services::alpha_arena::LiquidationEvent;
using fiorch::services::alpha_arena::OrderAck;
using fiorch::services::alpha_arena::OrderRequest;
...
void HyperliquidVenue::reconcile_tick() {
    ...
    client_->info(body, [self, comp_id](Result<QJsonDocument> r) {
        ...
        auto agents = fiorch::services::alpha_arena::AlphaArenaRepo::instance()
                          .agents_for(comp_id);
        if (agents.is_err()) return;
        for (const auto& agent : agents.value()) {
            auto local = fiorch::services::alpha_arena::AlphaArenaRepo::instance()
            ...
            for (const auto& p : local.value()) {
                ...
                if (std::fabs(r_qty - p.qty) > 1e-9) {
                    ...
                    fiorch::services::alpha_arena::AlphaArenaRepo::instance()

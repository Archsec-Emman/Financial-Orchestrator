...
QString session_resolve_script_path(const QString& relative) {
    const QString dir = fiorch::python::PythonRunner::instance().scripts_dir();
    if (dir.isEmpty())
        return {};
    return dir + "/" + relative;
}
...
bool session_is_broker_stream(const QString& id) {
    return fiorch::trading::BrokerRegistry::instance().has(id);
}
...
void ExchangeSession::set_credentials(const ExchangeCredentials& creds) {
    ...
    auto& ss = fiorch::SecureStorage::instance();

...
class InstrumentRepository : public fiorch::BaseRepository<Instrument> {
  public:
    ...
    fiorch::Result<void> replace_all(const QString& broker_id, const QVector<Instrument>& instruments);

    /// Clear all instruments for a broker.
    fiorch::Result<void> clear(const QString& broker_id);

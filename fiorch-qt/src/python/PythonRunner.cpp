...
// — encoding pins, FIORCH_DATA_DIR, FINAGENT_DATA_DIR, base PYTHONPATH — lives
...
QProcessEnvironment PythonRunner::build_python_env() const {
    ...
    env.insert("FIORCH_DATA_DIR", install_dir);

    // FINAGENT_DATA_DIR: default to <FIORCH_DATA_DIR>/finagent. Always overwrite
...
void PythonRunner::start_next() {
    ...
    while (active_count_ < max_concurrent_ && !queue_.isEmpty()) {
        ...
        // Start from the shared base env (encoding pins, FIORCH_DATA_DIR,

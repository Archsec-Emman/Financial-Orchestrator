...
void StatusBar::wire_link_indicator() {
    auto& ctx = fiorch::SymbolContext::instance();
    // Update on every active-symbol publish. Phase 7 already broadcasts
    // these process-wide via the SymbolContext singleton.
    connect(&ctx, &fiorch::SymbolContext::active_symbol_changed, this,
            [this](const fiorch::SymbolRef& ref, QObject*) {
                ...
                fiorch::SymbolGroup matched = fiorch::SymbolGroup::None;
                auto& sc = fiorch::SymbolContext::instance();
                for (auto g : fiorch::all_symbol_groups()) {
    ...
    connect(&ctx, &fiorch::SymbolContext::group_symbol_changed, this,
            [this](fiorch::SymbolGroup g, const fiorch::SymbolRef& ref, QObject*) {
...
void StatusBar::update_link_label(fiorch::SymbolGroup g, const fiorch::SymbolRef& ref) {
    if (!ref.is_valid()) {
        link_label_->setVisible(false);
        return;
    }
    QString text;
    if (g == fiorch::SymbolGroup::None) {
        text = ref.display();
    } else {
        text = QString("%1 · %2")
                   .arg(QChar(fiorch::symbol_group_letter(g)))
                   .arg(ref.display());
    }
    ...
    const QColor tint = (g == fiorch::SymbolGroup::None)

...
static const struct {
    ...
    {"🇱🇨", "Saint Lucia", "+1-758"},
    {"🇻🇨", "Saint Vincent & Grenadines", "+1-784"},
    {"🇼🇸", "Samoa", "+685"},
    {"🇸🇲", "San Marino", "+378"},
...
PhoneCodeComboBox::PhoneCodeComboBox(QWidget* parent) : QComboBox(parent) {
    ...
    connect(comp, QOverload<const QModelIndex&>::of(&QCompleter::activated), this, [this](const QModelIndex& idx) {
        int row = idx.row();
        setCurrentIndex(row);
        sync_display(row);
    });
    ...
    arrow_lbl_->setStyleSheet("color:#707070; font-size:16px; background:transparent;"
    ...
    if (auto* le = lineEdit()) {
        ...
        connect(le, &QLineEdit::textEdited, this, [this](const QString& text) {
            ...
            for (int i = 0; i < model_->rowCount(); i++) {
                if (model_->item(i)->data(Qt::UserRole).toString().compare(text.trimmed(), Qt::CaseInsensitive) == 0) {
                    setCurrentIndex(i);
                    return;
                }
            }

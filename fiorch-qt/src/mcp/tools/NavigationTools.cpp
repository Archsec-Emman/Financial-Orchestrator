...
fiorch::DockScreenRouter* find_active_router_main_thread() {
    // Prefer the focused window — that's where the user just typed.
    if (auto* active = QApplication::activeWindow()) {
        if (auto* mw = qobject_cast<fiorch::WindowFrame*>(active))
    ...
    for (QWidget* w : top_widgets) {
        if (auto* mw = qobject_cast<fiorch::WindowFrame*>(w)) {
            if (mw->window_id() == 0)
                return mw->dock_router();
        }
    }
        ...
        t.handler = [](const QJsonObject&) -> ToolResult {
            ...
            for (const auto& id : snap.ids) {
                tabs.append(QJsonObject{{"id", id},
                                        {"title", fiorch::DockScreenRouter::title_for_id(id)}});
            }
    ...
    // ── get_current_tab ─────────────────────────────────────────────────
    {
        ...
        t.handler = [](const QJsonObject&) -> ToolResult {
            ...
            return ToolResult::ok_data(QJsonObject{
                {"id", snap.current_id},
                {"title", fiorch::DockScreenRouter::title_for_id(snap.current_id)}});
        };
        tools.push_back(std::move(t));
    }

...
std::vector<ToolDef> get_profile_tools() {
    ...
    // ── profile_mark_notification_read ───────────────────────────────────────
    {
        ...
        t.handler = [](const QJsonObject& args) -> ToolResult {
            ...
            fiorch::notifications::NotificationService::instance().mark_read(id);
            QJsonObject result;
            result["success"] = true;
            result["id"] = id;
    ...
    // ── profile_mark_all_notifications_read ──────────────────────────────────
    {
        ...
        t.input_schema.properties = QJsonObject{};
        t.handler = [](const QJsonObject&) -> ToolResult {
            fiorch::notifications::NotificationService::instance().mark_all_read();

...
static void apply_tokens(const QString& api_key, const QString& session_token) {
    auto& http = fiorch::HttpClient::instance();
...
void AuthManager::save_session() {
    ...
    auto r = fiorch::SettingsRepository::instance().set("fincept_session", json, "auth");
    if (r.is_err()) {
    ...
    if (!session_.api_key.isEmpty()) {
        auto sr = fiorch::SecureStorage::instance().store("api_key", session_.api_key);
        if (sr.is_err())
...
void AuthManager::load_session() {
    auto r = fiorch::SettingsRepository::instance().get("fincept_session");
    ...
    auto secure_key = fiorch::SecureStorage::instance().retrieve("api_key");
...
void AuthManager::clear_session() {
    ...
    clear_tokens();
    fiorch::SettingsRepository::instance().remove("fincept_session");
    fiorch::SettingsRepository::instance().remove("fincept_api_key");
    fiorch::SecureStorage::instance().remove("api_key");
    ...
    LlmConfigRepository::instance().delete_provider("fiorch");
}
    ...
    AuthApi::instance().get_user_profile([this, on_done = std::move(on_done)](ApiResponse r) mutable {
        ...
        if (r.success) {
            ...
            if (!session_.session_token.isEmpty()) {
                fiorch::HttpClient::instance().set_session_token(session_.session_token);
            }
        } else {
...
void AuthManager::attempt_session_recovery(std::function<void(bool)> cb) {
    ...
    fiorch::HttpClient::instance().clear_session_token();
...
void AuthManager::auto_configure_fincept_llm() {
    ...
    fiorch::SettingsRepository::instance().set("fincept_api_key", session_.api_key, "auth");
    ...
    if (!has_active)
        LlmConfigRepository::instance().set_active("fiorch");
}

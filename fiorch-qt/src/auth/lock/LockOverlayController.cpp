...
fiorch::screens::LockScreen*
LockOverlayController::lock_screen_for(fiorch::WindowFrame* frame) {
    if (!frame)
        return nullptr;
    ...
    auto* ls = new fiorch::screens::LockScreen(nullptr);

    Tracked t;
    t.frame = QPointer<fiorch::WindowFrame>(frame);
    t.lock_screen = QPointer<fiorch::screens::LockScreen>(ls);
...
void LockOverlayController::on_frame_added(fiorch::WindowFrame* w) {
    if (!w)
        return;
    ...
    if (!tracked_frames_.contains(w))
        tracked_frames_.insert(w, Tracked{QPointer<fiorch::WindowFrame>(w), {}});
}

void LockOverlayController::on_frame_removing(fiorch::WindowFrame* w) {
    if (!w)
        return;

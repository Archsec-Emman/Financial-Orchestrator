...
class LockOverlayController : public QObject {
  ...
  public:
    ...
    fiorch::screens::LockScreen* lock_screen_for(fiorch::WindowFrame* frame);

    /// Test/debug accessor.
    int tracked_frame_count() const { return tracked_frames_.size(); }

  private slots:
    void on_frame_added(fiorch::WindowFrame* w);
    void on_frame_removing(fiorch::WindowFrame* w);
  ...
  private:
    ...
    /// frame teardown).
    struct Tracked {
        QPointer<fiorch::WindowFrame> frame;
        QPointer<fiorch::screens::LockScreen> lock_screen;
    };
    QHash<fiorch::WindowFrame*, Tracked> tracked_frames_;
};

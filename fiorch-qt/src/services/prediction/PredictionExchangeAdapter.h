...
class PredictionExchangeAdapter : public QObject {
  ...
  signals:
    // Read
    void markets_ready(const QVector<fiorch::services::prediction::PredictionMarket>& markets);
    void events_ready(const QVector<fiorch::services::prediction::PredictionEvent>& events);
    void search_results_ready(const QVector<fiorch::services::prediction::PredictionMarket>& markets,
                              const QVector<fiorch::services::prediction::PredictionEvent>& events);
    void tags_ready(const QStringList& tags);
    void market_detail_ready(const fiorch::services::prediction::PredictionMarket& market);
    void event_detail_ready(const fiorch::services::prediction::PredictionEvent& event);
    void order_book_ready(const fiorch::services::prediction::PredictionOrderBook& book);
    void price_history_ready(const fiorch::services::prediction::PriceHistory& history);
    void recent_trades_ready(const QVector<fiorch::services::prediction::PredictionTrade>& trades);
    ...
    void ws_orderbook_updated(const QString& asset_id, const fiorch::services::prediction::PredictionOrderBook& book);
    void ws_connection_changed(bool connected);

    // Account / trading
    void credentials_changed();  // emitted when user adds/removes a credential set
    void balance_ready(const fiorch::services::prediction::AccountBalance& balance);
    void positions_ready(const QVector<fiorch::services::prediction::PredictionPosition>& positions);
    void open_orders_ready(const QVector<fiorch::services::prediction::OpenOrder>& orders);
    void order_placed(const fiorch::services::prediction::OrderResult& result);

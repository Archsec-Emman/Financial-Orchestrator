...
namespace pm = fiorch::services::polymarket;
namespace pr = fiorch::services::prediction;
namespace pmap = fiorch::services::prediction::polymarket_map;

PolymarketAdapter::PolymarketAdapter(QObject* parent)
    : fiorch::services::prediction::PredictionExchangeAdapter(parent) {
...
void PolymarketAdapter::run_py(const QString& command, const QJsonObject& payload,
    ...
    fiorch::python::PythonRunner::instance().run(
        ...
        {command, payload_str},
        [self, on_ok, ctx](fiorch::python::PythonResult r) {
            if (!self) return;
            if (!r.success) {
                emit self->error_occurred(ctx, r.error.isEmpty() ? r.output : r.error);
                return;
            }
            const QString json_str = fiorch::python::extract_json(r.output);

...
namespace pr = fiorch::services::prediction;
...
KalshiWsClient::KalshiWsClient(QObject* parent) : QObject(parent) {
    ws_ = new fiorch::WebSocketClient(this);
    connect(ws_, &fiorch::WebSocketClient::connected, this, &KalshiWsClient::on_connected);
    connect(ws_, &fiorch::WebSocketClient::disconnected, this, &KalshiWsClient::on_disconnected);
    connect(ws_, &fiorch::WebSocketClient::message_received, this, &KalshiWsClient::on_message);
    connect(ws_, &fiorch::WebSocketClient::error_occurred, this, &KalshiWsClient::on_error);
...
void KalshiWsClient::ensure_connected() {
    ...
    // -SIGNATURE / -TIMESTAMP headers which fiorch::WebSocketClient
...
void KalshiWsClient::publish_price(const QString& asset_id, double price) {
    ...
    fiorch::datahub::DataHub::instance().publish(topic, QVariant(price));
}
...
void KalshiWsClient::publish_orderbook(const QString& asset_id, const pr::PredictionOrderBook& book) {
    ...
    fiorch::datahub::DataHub::instance().publish(topic, QVariant::fromValue(book));
}
...
void KalshiWsClient::ensure_registered_with_hub() {
    ...
    fiorch::datahub::TopicPolicy push_only;

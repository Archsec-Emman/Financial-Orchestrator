...
static constexpr const char* kApiBase = "https://api.fiorch.in/research/news-events";

namespace {
inline void publish_to_hub(const QString& topic, const QVariant& value) {
    fiorch::datahub::DataHub::instance().publish(topic, value);
}
}  // namespace
...
void GeopoliticsService::fetch_unique_countries() {
    ...
    const QVariant cached = fiorch::CacheManager::instance().get("geo:countries");
    if (!cached.isNull()) {
...
void GeopoliticsService::fetch_unique_categories() {
    const QVariant cached = fiorch::CacheManager::instance().get("geo:categories");
    if (!cached.isNull()) {
        const QJsonArray arr = QJsonDocument::fromJson(cached.toString().toUtf8()).array();
...
void GeopoliticsService::ensure_registered_with_hub() {
    ...
    fiorch::datahub::TopicPolicy events_policy;
    ...
    fiorch::datahub::TopicPolicy ref_policy;
    ...
    fiorch::datahub::TopicPolicy hdx_policy;
    ...
    fiorch::datahub::TopicPolicy push_policy;

...
void LlmService::ensure_config() const {
    ...
    if (config_loaded_) {
        if (provider_ == "fiorch") {
            const auto& sess = fiorch::auth::AuthManager::instance().session();
...
QString LlmService::get_endpoint_url() const {
    ...
    if (p == "fiorch") {
        return "https://api.fiorch.in/research/chat";
    }
...
QMap<QString, QString> LlmService::get_headers() const {
    ...
    if (p == "anthropic") {
        ...
        if (p == "openrouter") {
            ...
            h["X-Title"] = "Financial Orchestrator";
        }
    }
    return h;
}
...
LlmResponse LlmService::do_streaming_request(const QString& user_message,
    ...
    if (provider_ == "gemini" || provider_ == "google" || provider_ == "fiorch") {

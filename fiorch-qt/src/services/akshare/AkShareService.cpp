...
namespace fiorch::services::akshare {
...
void AkShareService::fetch_endpoints(const QString& script, EndpointsCallback cb) {
    fiorch::python::PythonRunner::instance().run(
        ...
        [cb = std::move(cb), script](const fiorch::python::PythonResult& result) {
            EndpointsResult out;
            if (!result.success) {
            ...
            const QString json_str = fiorch::python::extract_json(result.output);
            if (json_str.isEmpty()) {
...
void AkShareService::query(const QString& script, const QString& endpoint,
    ...
    fiorch::python::PythonRunner::instance().run(
        script, args,
        [cb = std::move(cb), script, endpoint](const fiorch::python::PythonResult& result) {
            QueryResult out;
            if (!result.success) {

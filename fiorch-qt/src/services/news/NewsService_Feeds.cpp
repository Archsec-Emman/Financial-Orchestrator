...
fiorch::RssFeedRow to_row(const fiorch::services::RSSFeed& f, bool is_builtin, bool enabled) {
    fiorch::RssFeedRow r;
    r.id = f.id;
    r.name = f.name;
    r.url = f.url;
...
fiorch::services::RSSFeed apply_overlay(const fiorch::services::RSSFeed& base,
                                          const fiorch::RssFeedRow& patch) {
    fiorch::services::RSSFeed merged = base;
...
QVector<RSSFeed> NewsService::list_effective_feeds() const {
    ...
    auto repo_res = fiorch::RssFeedRepository::instance().list_all();
    if (repo_res.is_err()) {
    ...
    QHash<QString, fiorch::RssFeedRow> by_id;
...
bool NewsService::add_user_feed(const RSSFeed& f) {
    ...
    auto r = fiorch::RssFeedRepository::instance().upsert(row);
    if (r.is_err()) {
...
bool NewsService::remove_or_reset_feed(const QString& id) {
    auto r = fiorch::RssFeedRepository::instance().remove(id);
    if (r.is_err()) {
...
bool NewsService::set_feed_enabled(const QString& id, bool enabled) {
    ...
    auto& repo = fiorch::RssFeedRepository::instance();
...
void NewsService::reload_feeds() {
    fiorch::CacheManager::instance().clear_category("news");
    emit feeds_changed();
}

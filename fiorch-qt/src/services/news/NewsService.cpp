...
void NewsService::fetch_all_news(bool force, ArticlesCallback cb) {
    if (!force) {
        const QVariant cached = fiorch::CacheManager::instance().get("news:articles");
        if (!cached.isNull()) {
...
void NewsService::summarize_headlines(const QVector<NewsArticle>& articles, int count, SummaryCallback cb) {
...

    {
        ...
        const QVariant cached = fiorch::CacheManager::instance().get(sum_key);
        if (!cached.isNull()) {
    ...
    HttpClient::instance().post("/news/summarize", body, [cb, sig](Result<QJsonDocument> result) {
        ...
        if (!summary.isEmpty()) {
            fiorch::CacheManager::instance().put("news:summary:" + sig.left(200), QVariant(summary),
...
void NewsService::ensure_registered_with_hub() {
    ...
    fiorch::datahub::TopicPolicy general;
    ...
    fiorch::datahub::TopicPolicy derived = general;
    ...
    fiorch::datahub::TopicPolicy cluster_policy;

...
QVector<NewsCluster> cluster_articles(const QVector<NewsArticle>& articles) {
    ...
    for (const auto& members : cluster_members) {
        ...
        for (int idx : members) {
            ...
            if (a.sort_ts > c.latest_sort_ts)
                c.latest_sort_ts = a.sort_ts;
            if (a.tier < c.tier)
                c.tier = a.tier;
            if (a.priority == Priority::FLASH || a.priority == Priority::URGENT)
                c.is_breaking = true;
            if (a.sentiment == Sentiment::BULLISH)
                bullish++;
            if (a.sentiment == Sentiment::BEARISH)
                bearish++;
        }
        ...
        c.sentiment = bullish > bearish   ? Sentiment::BULLISH
                      : bearish > bullish ? Sentiment::BEARISH
                                          : Sentiment::NEUTRAL;

        // Velocity: articles in last 2h vs previous 6h
        int recent = 0, older = 0;
        for (const auto& a : c.articles) {
            if ((now - a.sort_ts) < 7200)
                recent++;
            else
                older++;
        }

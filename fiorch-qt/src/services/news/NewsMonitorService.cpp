...
void NewsMonitorService::load_from_db() {
    monitors_.clear();
    auto result = fiorch::NewsMonitorRepository::instance().list_all();
    if (result.is_err())
        return;
...
void NewsMonitorService::save_to_db(const NewsMonitor& m) {
    fiorch::NewsMonitorRow row{m.id, m.label, m.keywords, m.color, m.enabled};
    fiorch::NewsMonitorRepository::instance().upsert(row);
}

void NewsMonitorService::remove_from_db(const QString& id) {
    fiorch::NewsMonitorRepository::instance().remove(id);
}

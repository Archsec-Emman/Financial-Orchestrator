...
void ClapDetectorService::start() {
    ...
    env.insert("FIORCH_DATA_DIR", python::PythonSetupManager::instance().install_dir());
    ...
    env.insert("FIORCH_CLAP_MODE", mode);
    env.insert("FIORCH_CLAP_PEAK_MIN", peak);
    env.insert("FIORCH_CLAP_PR_RATIO", ratio);
    env.insert("FIORCH_CLAP_MAX_GAP_MS", gap);
    env.insert("FIORCH_CLAP_DEBOUNCE_MS", debounce);
    env.insert("FIORCH_STT_DEVICE", device);

...
class OptionChainService : public QObject, public fiorch::datahub::Producer {
    Q_OBJECT
  public:
  ...
  signals:
    ...
    void chain_published(const fiorch::services::options::OptionChain& chain);
  ...
  private:
    ...
    void enrich_with_greeks(const fiorch::services::options::OptionChain& chain, const QString& topic);
    ...
    void publish_per_leg_ticks(const fiorch::services::options::OptionChain& chain);

    /// Average ATM CE/PE IV after Greeks land. Publishes
    /// `option:atm_iv:<broker>:<underlying>` (decimal IV).
    void publish_atm_iv(const fiorch::services::options::OptionChain& chain);
    ...
    QHash<qint64, fiorch::services::options::OptionGreeks> greeks_cache_;

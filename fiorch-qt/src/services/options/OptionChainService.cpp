...
namespace fiorch::services::options {

using fiorch::trading::BrokerCredentials;
using fiorch::trading::BrokerQuote;
using fiorch::trading::BrokerRegistry;
using fiorch::trading::IBroker;
using fiorch::trading::Instrument;
using fiorch::trading::InstrumentService;
using fiorch::trading::InstrumentType;
...
void OptionChainService::ensure_registered_with_hub() {
    ...
    fiorch::datahub::TopicPolicy chain_pol;
    ...
    fiorch::datahub::TopicPolicy iv_pol;
    ...
    fiorch::datahub::TopicPolicy derived_pol;
    ...
    fiorch::datahub::TopicPolicy tick_pol;
    ...
    for (const auto& topic : topics) {
        ...
        if (topic.startsWith(kChainPrefix)) {
            if (!parse_chain_topic(topic, broker, underlying, expiry)) {
                ...
                fiorch::datahub::DataHub::instance().publish_error(topic, "malformed topic");
                continue;
            }
...
void OptionChainService::refresh_chain(const QString& broker_id, const QString& underlying,
    ...
    if (!broker) {
        ...
        fiorch::datahub::DataHub::instance().publish_error(topic, "broker not registered: " + broker_id);
        return;
    }
    ...
    if (instruments.isEmpty()) {
        in_flight_.remove(topic);
        fiorch::datahub::DataHub::instance().publish_error(topic,
        ...
        QMetaObject::invokeMethod(
            ...
            [self, resp, instruments, broker_id, underlying, expiry, topic, underlying_sym]() {
                ...
                if (!resp.success || !resp.data.has_value()) {
                    fiorch::datahub::DataHub::instance().publish_error(topic, resp.error);
...
double OptionChainService::risk_free_rate() {
    ...
    auto r = fiorch::SettingsRepository::instance().get(QStringLiteral("fno.risk_free_rate"),
...
void OptionChainService::publish_atm_iv(const OptionChain& chain) {
    ...
    fiorch::datahub::DataHub::instance().publish(iv_topic, QVariant::fromValue(atm_iv));
    ...
    if (atm_iv > 0) {
        ...
        auto r = fiorch::IvHistoryRepository::instance().upsert(chain.underlying, today, atm_iv);
        if (r.is_err()) {
...
void OptionChainService::enrich_with_greeks(const OptionChain& chain, const QString& topic) {
    ...
    auto add_leg = [&](qint64 token, double strike, const fiorch::trading::BrokerQuote& quote, bool is_call) {
        if (token == 0)
            return;
    ...
    if (contracts.isEmpty()) {
        ...
        if (!iv_cache_.isEmpty() || !greeks_cache_.isEmpty()) {
            fiorch::datahub::DataHub::instance().publish(topic, QVariant::fromValue(enriched));
    ...
    QPointer<OptionChainService> self = this;
    fiorch::python::OptionGreeksWorker::instance().submit(

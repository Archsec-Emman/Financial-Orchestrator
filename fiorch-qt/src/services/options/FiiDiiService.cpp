...
void FiiDiiService::ensure_registered_with_hub() {
    ...
    fiorch::datahub::TopicPolicy pol;
    pol.ttl_ms = kTtlMs;
...
void FiiDiiService::kick_off_refresh() {
    ...
    fiorch::python::PythonRunner::instance().run(
        QStringLiteral("fii_dii_scraper.py"), {},
        [self](fiorch::python::PythonResult res) {
            if (!self)
                return;
            ...
            if (!res.success) {
                ...
                fiorch::datahub::DataHub::instance().publish_error(kTopic, res.error);
                emit self->failed(res.error);
                return;
            }

            const QString out = fiorch::python::extract_json(res.output);
            QJsonParseError pe;
            ...
            if (pe.error != QJsonParseError::NoError || !doc.isArray()) {
                ...
                LOG_WARN("FiiDii", err);
                fiorch::datahub::DataHub::instance().publish_error(kTopic, err);
            ...
            if (!fresh.isEmpty()) {
                auto upr = fiorch::FiiDiiRepository::instance().upsert_batch(fresh);
                if (upr.is_err()) {
            ...
            auto qr = fiorch::FiiDiiRepository::instance().get_recent(kPublishWindowDays);
            if (qr.is_err()) {
            ...
            fiorch::datahub::DataHub::instance().publish(kTopic, QVariant::fromValue(rows));

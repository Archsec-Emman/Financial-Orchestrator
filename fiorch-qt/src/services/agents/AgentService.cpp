...
void AgentService::clear_cache() {
    fiorch::CacheManager::instance().clear_category("agents");
    LOG_INFO("AgentService", "Cache cleared");
}

QVector<AgentInfo> AgentService::cached_agents() const {
    const QVariant cv = fiorch::CacheManager::instance().get("agents:list");
    if (cv.isNull())
        return {};
...
QJsonObject AgentService::build_api_keys() const {
    ...
    if (!keys.contains("fiorch")) {
        const auto& session = auth::AuthManager::instance().session();
        if (!session.api_key.isEmpty()) {
            keys["fiorch"] = session.api_key;
            keys["FIORCH_API_KEY"] = session.api_key;
        }
    }
...
void AgentService::run_python_stdin(const QString& action, const QJsonObject& params, const QJsonObject& config,
    ...
    // PythonRunner so every finagent spawn sees the same FIORCH_DATA_DIR,
...
bool AgentService::get_cached_response(const QString& key, QJsonObject& out) const {
    const QVariant cv = fiorch::CacheManager::instance().get("agents:resp:" + key);
    if (cv.isNull())
        return false;
...
void AgentService::set_cached_response(const QString& key, const QJsonObject& data) {
    fiorch::CacheManager::instance().put(
...
void AgentService::clear_response_cache() {
    fiorch::CacheManager::instance().remove_prefix("agents:resp:");
...
QJsonObject AgentService::get_cache_stats() const {
    ...
    stats["total_entries"] = fiorch::CacheManager::instance().entry_count();
    return stats;
}
...
void AgentService::ensure_registered_with_hub() {
    ...
    fiorch::datahub::TopicPolicy output_policy;
    ...
    fiorch::datahub::TopicPolicy stream_policy;
    ...
    fiorch::datahub::TopicPolicy status_policy;
    ...
    fiorch::datahub::TopicPolicy routing_policy;
    ...
    fiorch::datahub::TopicPolicy error_policy;
    ...
    fiorch::datahub::TopicPolicy task_event_policy;
...
void AgentService::publish_agent_token(const QString& run_id, const QString& token) {
    ...
    fiorch::datahub::DataHub::instance().publish(
...
void AgentService::publish_task_event(const QString& task_id, const QJsonObject& event) {
    ...
    fiorch::datahub::DataHub::instance().publish(topic, QVariant(event));

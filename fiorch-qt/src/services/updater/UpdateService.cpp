...
void UpdateService::on_manifest_reply_finished() {
    ...
    if (!is_newer(local_version, remote_version)) {
        ...
        if (!silent_) {
            QMessageBox::information(
                dialog_parent(), QStringLiteral("Financial Orchestrator"),
    ...
    QString prompt = QStringLiteral("A new version of Financial Orchestrator is available.\n\n"
                                    "Current version: %1\nLatest version:  %2\n\n")
...
void UpdateService::show_error(const QString& text) {
    QMessageBox::warning(dialog_parent(), QStringLiteral("Financial Orchestrator"), text);
}

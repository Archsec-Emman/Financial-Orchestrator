...
QHash<QString, fiorch::wallet::TierStatus::Tier>& tier_cache() {
    static QHash<QString, fiorch::wallet::TierStatus::Tier> c;
    return c;
}
...
fiorch::wallet::TierStatus::Tier
TierService::get_cached_tier(const QString& pubkey) const {
    return tier_cache().value(pubkey, fiorch::wallet::TierStatus::Tier::Free);
}
...
void TierService::publish_for(const QString& topic, const QString& pubkey) {
    ...
    bool any_data = false;
    if (v.canConvert<fiorch::wallet::VeFncptAggregate>()) {
    ...
    const auto prev = cache.value(pubkey, fiorch::wallet::TierStatus::Tier::Free);
...
void TierService::wire_vefncpt_listener() {
    ...
    auto& hub = fiorch::datahub::DataHub::instance();
    QObject::connect(&hub, &fiorch::datahub::DataHub::topic_updated, this,

...
const QString QuantLibClient::API_BASE = QStringLiteral("https://api.fiorch.in");
    ...
    if (cacheable) {
        ...
        connect(reply, &QNetworkReply::finished, this, [self, reply, nam, endpoint, cache_key, callback]() {
            ...
            if (result.success) {
                ...
                if (!to_cache.isEmpty())
                    fiorch::CacheManager::instance().put(cache_key, QVariant(to_cache), kRefDataTtlSec, "quantlib");
            }
            LOG_DEBUG(
                kQuantLibClientTag,

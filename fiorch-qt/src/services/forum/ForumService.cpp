...
void ForumService::fetch_categories(CategoriesCallback cb) {
    const QVariant cached = fiorch::CacheManager::instance().get("forum:categories");
    if (!cached.isNull()) {
...
void ForumService::fetch_stats(StatsCallback cb) {
    const QVariant cached = fiorch::CacheManager::instance().get("forum:stats");
    if (!cached.isNull()) {
...
void ForumService::fetch_trending(PostsCallback cb) {
    const QVariant cached = fiorch::CacheManager::instance().get("forum:trending");
    if (!cached.isNull()) {
        auto data = QJsonDocument::fromJson(cached.toString().toUtf8()).object();
        ForumPostsPage result;

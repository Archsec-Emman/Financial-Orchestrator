...
void EquityResearchService::load_quote_only(const QString& symbol) {
    if (symbol.isEmpty())
        return;
    const QVariant qcv = fiorch::CacheManager::instance().get("equity:quote:" + symbol);
    if (!qcv.isNull()) {
...
void EquityResearchService::load_info_only(const QString& symbol) {
    if (symbol.isEmpty())
        return;
    const QVariant icv = fiorch::CacheManager::instance().get("equity:info:" + symbol);
    if (!icv.isNull()) {
...
void EquityResearchService::load_historical_only(const QString& symbol, const QString& period) {
    if (symbol.isEmpty())
        return;
    const QVariant hcv = fiorch::CacheManager::instance().get("equity:candles:" + symbol);
    if (!hcv.isNull()) {
    ...
    if (!cached_json.isEmpty()) {
        ...
        run_python("yfinance_data.py", {"historical_period", symbol, period, "1d"},
                   [this, symbol, run_compute](bool ok, const QString& out) {
                       ...
                       fiorch::CacheManager::instance().put("equity:candles:" + symbol, QVariant(raw),
                                                             kHistoricalTtlSec, "equity");
                       run_compute(raw);
                   });
    }
...
void EquityResearchService::fetch_news(const QString& symbol, int count) {
    {
        const QVariant ncv = fiorch::CacheManager::instance().get("equity:news:" + symbol);
        if (!ncv.isNull()) {
...
void EquityResearchService::compute_talipp(const QString& symbol, const QString& indicator, const QVariantMap& params,
    ...
    const QVariant cached_candles = fiorch::CacheManager::instance().get("equity:candles:" + symbol);

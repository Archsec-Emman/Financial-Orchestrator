        ...
        .parameters =
            {
                ...
                {"llm_profile_id",
                 ...
                 "",
                 {},
                 "Override LLM profile (leave blank to use agent default)"},
                {"extra_instructions",
                 "Extra Instructions",
                 "code",
                 "",
        ...
        .execute =
            [](const QJsonObject& params, const QVector<QJsonValue>& inputs,
               std::function<void(bool, QJsonValue, QString)> cb) {
                ...
                QJsonArray tools;
                if (!agent_id.isEmpty()) {
                    auto res = fiorch::AgentConfigRepository::instance().get(agent_id);
                    if (res.is_ok()) {
                ...
                if (!extra_instr.isEmpty()) {
                    instructions = instructions.isEmpty() ? extra_instr : instructions + "\n\n" + extra_instr;
                }

                // Resolve LLM profile
                using fiorch::ai_chat::LlmService;
                using fiorch::LlmProfileRepository;
                using fiorch::ResolvedLlmProfile;
                ...
                guard->conn = QObject::connect(
                    &svc, &fiorch::services::AgentService::agent_result, guard,
                    [guard](const fiorch::services::AgentExecutionResult& result) {
                        if (result.request_id != guard->req_id)
                            return;
                        QObject::disconnect(guard->conn);
                        guard->deleteLater();
                        if (!result.success) {
                            guard->cb(false, {}, result.error.isEmpty() ? "Agent execution failed" : result.error);
                            return;
                        }
                        QJsonObject out;
        ...
        .execute =
            [](const QJsonObject& params, const QVector<QJsonValue>& inputs,
               std::function<void(bool, QJsonValue, QString)> cb) {
                ...
                QString system_prompt = "You are a tool-selection assistant for the Financial Orchestrator.\n"
                ...
                (void)QtConcurrent::run([system_prompt, query, cb]() {
                    using fiorch::ai_chat::LlmService;
                    using fiorch::ai_chat::ConversationMessage;

                    // Single non-streaming call using globally active LLM config
                    auto resp = LlmService::instance().chat(query, {ConversationMessage{"system", system_prompt}});

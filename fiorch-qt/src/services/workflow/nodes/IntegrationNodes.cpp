...
using fiorch::python::PythonRunner;
            ...
            [](const QJsonObject& params, const QVector<QJsonValue>& inputs,
               std::function<void(bool, QJsonValue, QString)> cb) {
                ...
                PythonRunner::instance().run(
                    "spreadsheet.py", {"--args", args_json}, [cb](const fiorch::python::PythonResult& res) {
                        if (!res.success) {
                            cb(false, {}, res.error.isEmpty() ? "spreadsheet.py failed" : res.error);
                            return;
                        }
                        QString out = fiorch::python::extract_json(res.output).trimmed();

...
namespace fiorch::services::crypto {
...
void TotpService::generate(const QString& secret, TotpCallback cb) {
    ...
    const QString script =
        fiorch::python::PythonRunner::instance().scripts_dir() + "/exchange/totp_gen.py";

    fiorch::python::PythonRunner::instance().run(
        script, {secret},
        [cb = std::move(cb)](const fiorch::python::PythonResult& result) {
            TotpResult out;
            if (!result.success) {

...
static void wire_app_lifecycle(QApplication& app, fiorch::InstanceLock& lock) {
    QObject::connect(&lock, &fiorch::InstanceLock::message_received,
                     [](const QStringList& /*args*/) {
                         ...
                         fiorch::WindowCycler::instance().new_window_on_next_monitor();
    ...
    QObject::connect(&app, &QApplication::lastWindowClosed, &app, []() {
        ...
        const auto r = fiorch::SettingsRepository::instance().get(
        ...
        if (choice == QStringLiteral("show_launchpad")) {
            fiorch::screens::LaunchpadScreen::instance()->surface();
        } else {
...
int main(int argc, char* argv[]) {
    ...
    //      profiles can run simultaneously as independent primary instances
    {
        for (int i = 1; i < argc - 1; ++i) {
            if (qstrcmp(argv[i], "--profile") == 0) {
                fiorch::ProfileManager::instance().set_active(QString::fromUtf8(argv[i + 1]));
                break;
            }
        }
        ...
        QDir().mkpath(fiorch::AppPaths::root());
    }
    ...
    app.setApplicationName("FinancialOrchestrator");
#ifndef FIORCH_VERSION_STRING
#    define FIORCH_VERSION_STRING "0.0.0-dev"
#endif
    app.setApplicationVersion(QStringLiteral(FIORCH_VERSION_STRING));
    ...
    if (lock_status == fiorch::InstanceLock::Status::Secondary) {
#ifdef Q_OS_WIN
    ...
    fiorch::TerminalShell::instance().initialise();
    QObject::connect(&app, &QCoreApplication::aboutToQuit,
                     []() { fiorch::TerminalShell::instance().shutdown(); });
    ...
    fiorch::datahub::register_metatypes();
    // SymbolContext payload types — signals cross threads when a producer
    // service (not just UI) publishes a group change.
    qRegisterMetaType<fiorch::SymbolRef>("fiorch::SymbolRef");
    qRegisterMetaType<fiorch::SymbolGroup>("fiorch::SymbolGroup");
    ...
    fiorch::ComponentCatalog::instance().load_with_fallbacks({
    ...
    fiorch::services::MarketDataService::instance().ensure_registered_with_hub();
    ...
    fiorch::services::NewsService::instance().ensure_registered_with_hub();
    fiorch::services::EconomicsService::instance().ensure_registered_with_hub();
    fiorch::services::MacroCalendarService::instance().ensure_registered_with_hub();
    fiorch::trading::DataStreamManager::instance().ensure_registered_with_hub();
    fiorch::services::geo::GeopoliticsService::instance().ensure_registered_with_hub();
    fiorch::services::maritime::MaritimeService::instance().ensure_registered_with_hub();
    fiorch::services::maritime::PortsCatalog::instance().ensure_registered_with_hub();
    fiorch::services::RelationshipMapService::instance().ensure_registered_with_hub();
    fiorch::services::ma::MAAnalyticsService::instance().ensure_registered_with_hub();
    fiorch::wallet::TokenMetadataService::instance().load_from_storage();
    ...
    QTimer::singleShot(0, qApp, []() {
        ...
        add_quotes(fiorch::services::MarketDataService::indices_symbols());
        add_quotes(fiorch::services::MarketDataService::forex_symbols());
        add_quotes(fiorch::services::MarketDataService::crypto_symbols());
        add_quotes(fiorch::services::MarketDataService::commodity_symbols());
    ...
    QTimer::singleShot(0, qApp, []() {
        ...
        fiorch::services::options::OptionChainService::instance().ensure_registered_with_hub();
        ...
        fiorch::services::options::OISnapshotter::instance().ensure_registered_with_hub();
        // F&O FII/DII flows — daily NSE cash-market institutional buy/sell.
        fiorch::services::options::FiiDiiService::instance().ensure_registered_with_hub();
        // Multi-broker session manager — `ws:kraken:*` / `ws:hyperliquid:*`.
        fiorch::trading::ExchangeSessionManager::instance().ensure_registered_with_hub();
        // Prediction Markets — `prediction:polymarket:*`.
        fiorch::services::polymarket::PolymarketWebSocket::instance().ensure_registered_with_hub();
        ...
        fiorch::services::alpha_arena::AlphaArenaEngine::instance().init();
        {
            auto& reg = fiorch::services::prediction::PredictionExchangeRegistry::instance();
            reg.register_adapter(
                std::make_unique<fiorch::services::prediction::polymarket_ns::PolymarketAdapter>());
            reg.register_adapter(
            ...
            if (auto* ks = dynamic_cast<fiorch::services::prediction::kalshi_ns::KalshiAdapter*>(
                    reg.adapter(QStringLiteral("kalshi")))) {
                if (auto creds = fiorch::services::prediction::PredictionCredentialStore::load_kalshi()) {
                    ks->set_credentials(*creds);
                }
            }
            if (auto* fi = reg.adapter(QStringLiteral("fiorch"))) {
                fi->ensure_registered_with_hub();
            }
        }
        // Specialized data sources.
        fiorch::services::DBnomicsService::instance().ensure_registered_with_hub();
        fiorch::services::GovDataService::instance().ensure_registered_with_hub();
        // Agents — `agent:*` push-only producer.
        fiorch::services::AgentService::instance().ensure_registered_with_hub();
        // Token metadata refresh — network call to Jupiter aggregator.
        fiorch::wallet::TokenMetadataService::instance().refresh_from_jupiter_async();
        // Wallet — `wallet:balance:*`, `market:price:token:*`.
        fiorch::wallet::WalletService::instance().ensure_registered_with_hub();
        fiorch::wallet::WalletService::instance().restore_from_storage();

        // Fee-discount eligibility producer (paid screens only).
        {
            static fiorch::billing::FeeDiscountService discount_service;
            auto& hub = fiorch::datahub::DataHub::instance();
            hub.register_producer(&discount_service);
            fiorch::datahub::TopicPolicy p;
            p.ttl_ms = 60 * 1000;
        ...
        {
            static fiorch::wallet::BuybackBurnService buyback_burn_service;
            static fiorch::wallet::TreasuryService treasury_service;
            auto& hub = fiorch::datahub::DataHub::instance();
            hub.register_producer(&buyback_burn_service);
            hub.register_producer(&treasury_service);

            fiorch::datahub::TopicPolicy epoch_p;
            ...
            fiorch::datahub::TopicPolicy reserves_p;
            ...
            fiorch::datahub::TopicPolicy runway_p;
        ...
        {
            auto& hub = fiorch::datahub::DataHub::instance();
            hub.register_producer(&fiorch::wallet::StakingService::instance());
            hub.register_producer(&fiorch::wallet::RealYieldService::instance());
            hub.register_producer(&fiorch::billing::TierService::instance());

            fiorch::datahub::TopicPolicy locks_p;
            ...
            fiorch::datahub::TopicPolicy tier_p;
    ...
    // Create all application directories under %LOCALAPPDATA%/com.fiorch.terminal
    fiorch::AppPaths::ensure_all();

    // ── One-time migration from legacy %APPDATA% location ─────────────────
    // Current locations (under %LOCALAPPDATA%\com.fiorch.terminal\):
    //   Log: <root>/logs/fincept.log    DB: <root>/data/fincept.db
    {
        ...
        migrate_file(old_base + "/fincept.db", fiorch::AppPaths::data() + "/fincept.db");
        migrate_file(old_base + "/cache.db", fiorch::AppPaths::data() + "/cache.db");
        migrate_file(old_base + "/fincept.log", fiorch::AppPaths::logs() + "/fincept.log");
        migrate_file(old_base + "/fincept-files", fiorch::AppPaths::files());
    ...
    fiorch::Logger::instance().set_file(fiorch::AppPaths::logs() + "/fincept.log");
    ...
    qInstallMessageHandler([](QtMsgType type, const QMessageLogContext& ctx, const QString& msg) {
        ...
        switch (type) {
        case QtDebugMsg:    fiorch::Logger::instance().debug(category, msg); break;
        case QtInfoMsg:     fiorch::Logger::instance().info(category, msg); break;
        case QtWarningMsg:  fiorch::Logger::instance().warn(category, msg); break;
        case QtCriticalMsg: fiorch::Logger::instance().error(category, msg); break;
        case QtFatalMsg:
            fiorch::Logger::instance().error(category, msg);
            fiorch::Logger::instance().flush_and_close();
            break;
        }
    });
    {
        auto& log = fiorch::Logger::instance();
        auto& cfg = fiorch::AppConfig::instance();

        // Global level
        const QString gl = cfg.get("log/global_level", "Info").toString();
        const QHash<QString, fiorch::LogLevel> lvl_map = {{"Trace", fiorch::LogLevel::Trace},
                                                           {"Debug", fiorch::LogLevel::Debug},
                                                           {"Info", fiorch::LogLevel::Info},
                                                           {"Warn", fiorch::LogLevel::Warn},
                                                           {"Error", fiorch::LogLevel::Error},
                                                           {"Fatal", fiorch::LogLevel::Fatal}};
        log.set_level(lvl_map.value(gl, fiorch::LogLevel::Info));
    ...
    LOG_INFO("App", "Financial Orchestrator v4.0.3 starting...");
    ...
    // Initialize config
    auto& config = fiorch::AppConfig::instance();
    fiorch::HttpClient::instance().set_base_url(config.api_base_url());
    ...
    fiorch::register_migration_v001();
    fiorch::register_migration_v002();
    fiorch::register_migration_v003();
    fiorch::register_migration_v004();
    fiorch::register_migration_v005();
    fiorch::register_migration_v006();
    fiorch::register_migration_v007();
    fiorch::register_migration_v008();
    fiorch::register_migration_v009();
    fiorch::register_migration_v010();
    fiorch::register_migration_v011();
    fiorch::register_migration_v012();
    fiorch::register_migration_v013();
    fiorch::register_migration_v014();
    fiorch::register_migration_v015();
    fiorch::register_migration_v016();
    fiorch::register_migration_v017();
    fiorch::register_migration_v018();
    fiorch::register_migration_v019();
    fiorch::register_migration_v020();
    fiorch::register_migration_v021();
    fiorch::register_migration_v022();
    fiorch::register_migration_v023();
    fiorch::register_migration_v024();
    fiorch::register_migration_v025();
    fiorch::register_migration_v026();
    fiorch::register_migration_v027();
    fiorch::register_migration_v028();
    fiorch::register_migration_v029();
    fiorch::register_migration_v030();
    fiorch::register_migration_v031();

    // Open main database
    QString db_path = fiorch::AppPaths::data() + "/fincept.db";
    auto db_result = fiorch::Database::instance().open(db_path);
    if (db_result.is_err()) {
        ...
        fiorch::ui::apply_global_stylesheet();
    } else {
        ...
        // to the main thread's event queue instead.
        {
            ...
            QTimer::singleShot(0, [news_cutoff]() {
                fiorch::NewsArticleRepository::instance().prune_older_than(news_cutoff);
        ...
        {
            auto& repo = fiorch::SettingsRepository::instance();
            auto& tm = fiorch::ui::ThemeManager::instance();
        ...
        fiorch::i18n::LanguageManager::instance().initialize();
    }

    // Open cache database (non-fatal if fails)
    QString cache_path = fiorch::AppPaths::data() + "/cache.db";
    auto cache_result = fiorch::CacheDatabase::instance().open(cache_path);
    ...
    // This lets us distinguish cross-session restores from same-session saves.
    {
        ...
        fiorch::ScreenStateManager::instance().set_session_id(sid);
    ...
    LOG_INFO("App", "Checking settings for legacy migration...");
    {
        ...
        auto existing = fiorch::SettingsRepository::instance().get("fincept_session");
        ...
        if (new_db_empty) {
            ...
            QString old_db_path = local_base.section('/', 0, -3) + "/FinancialOrchestrator/fincept_settings.db";
            ...
            if (QFile::exists(old_db_path)) {
                ...
                if (old_db.open()) {
                    ...
                    if (src.exec("SELECT key, value FROM settings")) {
                        int count = 0;
                        while (src.next()) {
                            fiorch::SettingsRepository::instance().set(src.value(0).toString(),
    ...
    // Start session
    fiorch::SessionManager::instance().start_session();
    ...
    fiorch::TerminalShell::instance().bootstrap_auth();
    ...
    fiorch::auth::SessionGuard session_guard;
    ...
    (void)fiorch::services::ReportBuilderService::instance();
    ...
    auto setup_status = fiorch::python::PythonSetupManager::instance().check_status();
    ...
    if (setup_status.needs_setup) {
        ...
        auto* setup_screen = new fiorch::screens::SetupScreen;
        QPointer<fiorch::screens::SetupScreen> screen_guard(setup_screen);
        setup_screen->setWindowTitle("Financial Orchestrator — First-Time Setup");
        ...
        QObject::connect(setup_screen, &fiorch::screens::SetupScreen::setup_complete,
            ...
            fiorch::KeyConfigManager::instance(); // init before WindowFrame registers shortcuts
            ...
            if (auto* recovery = fiorch::TerminalShell::instance().crash_recovery();
                recovery && recovery->needs_recovery()) {
                fiorch::screens::CrashRecoveryDialog dlg(
                    recovery, fiorch::TerminalShell::instance().snapshot_ring());
                dlg.exec();
                recovered = dlg.was_restored();
            }

            if (!recovered) {
                auto* window = new fiorch::WindowFrame(0); // primary window
            ...
            if (!recovered)
            {
                const QList<int> saved_ids =
                    fiorch::SessionManager::instance().load_window_ids();
                for (int id : saved_ids) {
                    if (id <= 0) continue; // 0 = primary, already created
                    auto* w = new fiorch::WindowFrame(id);
            ...
            if (!fiorch::ai_chat::LlmService::instance().is_configured())
                LOG_WARN("App",
            ...
            QTimer::singleShot(0, &app, []() {
                fiorch::services::AgentService::instance().discover_agents();
            });
    ...
    fiorch::KeyConfigManager::instance();
    ...
    if (!recovered) {
        auto* primary = new fiorch::WindowFrame(0);
    ...
    if (setup_status.needs_package_sync) {
        ...
        auto& mgr = fiorch::python::PythonSetupManager::instance();
        QObject::connect(&mgr, &fiorch::python::PythonSetupManager::setup_complete,

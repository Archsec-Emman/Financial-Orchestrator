...
WindowFrame::WindowFrame(int window_id, QWidget* parent, const WindowId& adopted_uuid)
    ...
    // "Financial Orchestrator" is the product brand and is intentionally not
    // translated — the bracketed profile name is the only variable part.
    const QString profile = ProfileManager::instance().active();
    setWindowTitle(profile == "default" ? QStringLiteral("Financial Orchestrator")
                                        : QStringLiteral("Financial Orchestrator [%1]").arg(profile));
...
void WindowFrame::update_window_title() {
    ...
    QString title = QStringLiteral("Financial Orchestrator");

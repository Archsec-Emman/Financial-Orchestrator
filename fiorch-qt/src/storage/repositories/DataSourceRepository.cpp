...
DataSource DataSourceRepository::map_row(QSqlQuery& q) {
    return {q.value(0).toString(), q.value(1).toString(), q.value(2).toString(),  q.value(3).toString(),
            ...
            q.value(8).toBool(),   q.value(9).toString(), q.value(10).toString(), q.value(11).toString()};
}
...
Result<QVector<DataSource>> DataSourceRepository::list_all() {
    return query_list(QString("SELECT %1 FROM data_sources ORDER BY display_name").arg(kDsCols), {}, map_row);
}

Result<DataSource> DataSourceRepository::get(const QString& id) {
    return query_one(QString("SELECT %1 FROM data_sources WHERE id = ?").arg(kDsCols), {id}, map_row);
}

...
using fiorch::services::Impact;
using fiorch::services::NewsArticle;
using fiorch::services::Priority;
using fiorch::services::Sentiment;
using fiorch::services::SourceFlag;
using fiorch::services::ThreatLevel;
...
Result<QVector<fiorch::services::NewsArticle>> NewsArticleRepository::load_saved() const {
    return query_list("SELECT id, headline, summary, source, region, category, link, sort_ts, "
                      "       priority, sentiment, impact, tickers, tier, lang, "
                      "       threat_level, threat_cat, threat_conf, source_flag "
...
Result<QVector<fiorch::services::NewsArticle>> NewsArticleRepository::search_fts(const QString& query,
    ...
    if (r.is_ok()) {
        QVector<fiorch::services::NewsArticle> result;
        auto& q = r.value();
        while (q.next())
            result.append(map_row(q));
        return Result<QVector<fiorch::services::NewsArticle>>::ok(std::move(result));
    }

...
class OISnapshotsRepository : public BaseRepository<fiorch::services::options::OISample> {
  public:
    ...
    Result<void> upsert_batch(qint64 token, const QVector<fiorch::services::options::OISample>& samples);

    /// Most recent N samples for a token (ts_minute DESC).
    Result<QVector<fiorch::services::options::OISample>> get_recent(qint64 token, int limit);

    /// Time-windowed read, ordered ascending — suitable for charting.
    /// Bounds inclusive. `until_minute=0` means "now".
    Result<QVector<fiorch::services::options::OISample>> get_window(qint64 token, qint64 since_minute,
  ...
  private:
    ...
    static fiorch::services::options::OISample map_row(QSqlQuery& q);
};

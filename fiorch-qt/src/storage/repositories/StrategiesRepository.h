...
struct SavedStrategyRow {
    qint64 id = 0;
    fiorch::services::options::Strategy strategy;
};
...
class StrategiesRepository : public BaseRepository<SavedStrategyRow> {
  public:
    ...
    Result<qint64> save(const fiorch::services::options::Strategy& s);

    /// Update row `id` in place. Bumps modified_at.
    Result<void> update(qint64 id, const fiorch::services::options::Strategy& s);
  ...
  private:
    ...
    static QString legs_to_json(const QVector<fiorch::services::options::StrategyLeg>& legs);
    static QVector<fiorch::services::options::StrategyLeg> legs_from_json(const QString& json);
};

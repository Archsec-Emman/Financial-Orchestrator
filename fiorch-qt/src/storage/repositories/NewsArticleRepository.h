...
class NewsArticleRepository : public BaseRepository<fiorch::services::NewsArticle> {
  public:
    ...
    Result<void> upsert_batch(const QVector<fiorch::services::NewsArticle>& articles);

    /// Load articles newer than since_ts (unix seconds).
    /// Optional category filter ("" = all). Ordered newest-first.
    Result<QVector<fiorch::services::NewsArticle>> load_recent(int64_t since_ts, const QString& category = {},
    ...
    Result<QVector<fiorch::services::NewsArticle>> load_saved() const;
    ...
    Result<QVector<fiorch::services::NewsArticle>> search_fts(const QString& query, int64_t since_ts = 0,
                                                               int limit = 500) const;

  private:
    NewsArticleRepository() = default;
    static fiorch::services::NewsArticle map_row(QSqlQuery& q);
};

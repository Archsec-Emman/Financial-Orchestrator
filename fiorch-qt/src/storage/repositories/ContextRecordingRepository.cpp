...
static const char* kCols = "id, tab_name, data_type, label, raw_data, metadata, data_size, tags, created_at";

Result<QVector<RecordedContext>> ContextRecordingRepository::list_all(int limit) {
    return query_list(
        QString("SELECT %1 FROM recorded_contexts ORDER BY created_at DESC LIMIT %2").arg(kCols).arg(limit), {},
        map_row);
}

Result<QVector<RecordedContext>> ContextRecordingRepository::list_by_tab(const QString& tab_name, int limit) {
    return query_list(QString("SELECT %1 FROM recorded_contexts WHERE tab_name = ? ORDER BY created_at DESC LIMIT %2")
                          .arg(kCols)
                          .arg(limit),
                      {tab_name}, map_row);
}
...
Result<RecordedContext> ContextRecordingRepository::get(const QString& id) {
    return query_one(QString("SELECT %1 FROM recorded_contexts WHERE id = ?").arg(kCols), {id}, map_row);
}
...
Result<void> ContextRecordingRepository::save(const RecordedContext& ctx) {
    return exec_write(
        ...
        "VALUES (?, ?, ?, ?, ?, ?, ?, ?)",
        {ctx.id, ctx.tab_name, ctx.data_type, ctx.label, ctx.raw_data, ctx.metadata, ctx.data_size, ctx.tags});
}

...
Result<void> apply_v001(QSqlDatabase& db) {
    ...
    // ── Settings ─────────────────────────────────────────────────────────────
    auto r = sql(db, "CREATE TABLE IF NOT EXISTS settings ("
    ...
    r = sql(db, "CREATE TABLE IF NOT EXISTS credentials ("
    ...
    r = sql(db, "CREATE TABLE IF NOT EXISTS key_value_storage ("
                "  key TEXT PRIMARY KEY,"
                "  value TEXT NOT NULL,"
    ...
    // ── Watchlists (parent) ──────────────────────────────────────────────────
    r = sql(db, "CREATE TABLE IF NOT EXISTS watchlists ("
                "  id TEXT PRIMARY KEY,"
                "  name TEXT NOT NULL,"
    ...
    r = sql(db, "CREATE TABLE IF NOT EXISTS watchlist_stocks ("
                "  id INTEGER PRIMARY KEY AUTOINCREMENT,"
    ...
    // ── Financial Notes ──────────────────────────────────────────────────────
    r = sql(db, "CREATE TABLE IF NOT EXISTS financial_notes ("
                "  id INTEGER PRIMARY KEY AUTOINCREMENT,"
                "  title TEXT NOT NULL,"
    ...
    // ── Note Templates ───────────────────────────────────────────────────────
    r = sql(db, "CREATE TABLE IF NOT EXISTS note_templates ("
                "  id INTEGER PRIMARY KEY AUTOINCREMENT,"
                "  name TEXT NOT NULL,"
                "  description TEXT,"
    ...
    // ── Reports ──────────────────────────────────────────────────────────────
    r = sql(db, "CREATE TABLE IF NOT EXISTS reports ("
                "  id INTEGER PRIMARY KEY AUTOINCREMENT,"
                "  title TEXT NOT NULL,"
                "  content_json TEXT,"
                "  created_at TEXT DEFAULT (datetime('now')),"
    ...
    // ── Report Templates ─────────────────────────────────────────────────────
    r = sql(db, "CREATE TABLE IF NOT EXISTS report_templates ("
                "  id TEXT PRIMARY KEY,"
                "  name TEXT NOT NULL,"
                "  description TEXT,"
    ...
    r = sql(db, "CREATE TABLE IF NOT EXISTS portfolio_holdings ("
                "  id INTEGER PRIMARY KEY AUTOINCREMENT,"
                "  symbol TEXT NOT NULL,"
                "  name TEXT DEFAULT '',"
    ...
    r = sql(db, "CREATE TABLE IF NOT EXISTS pt_portfolios ("
                "  id TEXT PRIMARY KEY,"
                "  name TEXT NOT NULL,"
    ...
    r = sql(db, "CREATE TABLE IF NOT EXISTS pt_orders ("
                "  id TEXT PRIMARY KEY,"
    ...
    r = sql(db, "CREATE TABLE IF NOT EXISTS pt_positions ("
                "  id TEXT PRIMARY KEY,"
                "  portfolio_id TEXT NOT NULL REFERENCES pt_portfolios(id) ON DELETE CASCADE,"
                "  symbol TEXT NOT NULL,"
                "  side TEXT NOT NULL,"
    ...
    r = sql(db, "CREATE TABLE IF NOT EXISTS pt_trades ("
                "  id TEXT PRIMARY KEY,"
                "  portfolio_id TEXT NOT NULL REFERENCES pt_portfolios(id) ON DELETE CASCADE,"
    ...
    r = sql(db, "CREATE TABLE IF NOT EXISTS pt_margin_blocks ("
                "  id TEXT PRIMARY KEY,"
                "  portfolio_id TEXT NOT NULL REFERENCES pt_portfolios(id) ON DELETE CASCADE,"
    ...
    r = sql(db, "CREATE TABLE IF NOT EXISTS pt_holdings ("
                "  id TEXT PRIMARY KEY,"
                "  portfolio_id TEXT NOT NULL REFERENCES pt_portfolios(id) ON DELETE CASCADE,"
                "  symbol TEXT NOT NULL,"
    ...
    r = sql(db, "CREATE TABLE IF NOT EXISTS news_monitors ("
                "  id TEXT PRIMARY KEY,"
                "  label TEXT NOT NULL,"
    ...
    // ── User RSS Feeds ───────────────────────────────────────────────────────
    r = sql(db, "CREATE TABLE IF NOT EXISTS user_rss_feeds ("
                "  id TEXT PRIMARY KEY,"
                "  name TEXT NOT NULL,"
    ...
    {
        QSqlQuery check(db);
        if (check.exec("SELECT name FROM sqlite_master WHERE type='table' AND name='watchlist'") && check.next()) {
            // Old table exists — migrate data
            QSqlQuery migrate(db);

            // Create a default watchlist
            migrate.exec("INSERT OR IGNORE INTO watchlists (id, name, color, is_default) "
                         "VALUES ('default', 'Default', '#FF6600', 1)");

            // Copy symbols
            migrate.exec("INSERT OR IGNORE INTO watchlist_stocks (watchlist_id, symbol, name, added_at) "
    ...
    {
        QSqlQuery check(db);
        if (check.exec("SELECT name FROM sqlite_master WHERE type='table' AND name='notes'") && check.next()) {
            QSqlQuery migrate(db);
            migrate.exec("INSERT OR IGNORE INTO financial_notes (title, content, category, created_at, updated_at) "

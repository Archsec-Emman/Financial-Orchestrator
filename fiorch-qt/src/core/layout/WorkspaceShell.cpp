...
Workspace WorkspaceShell::capture(const QString& name, const QString& kind,
    ...
    const auto frames = fiorch::WindowRegistry::instance().frames();
    ...
    const MonitorTopologyKey current = fiorch::MonitorTopology::current_key();
    ...
    for (auto* frame : frames) {
        ...
        if (auto* scr = frame->screen())
            variant.frame_screens.insert(id, fiorch::MonitorTopology::stable_id(scr));
    }
...
int WorkspaceShell::apply(const Workspace& workspace) {
    ...
    int applied = 0;
    auto frames_now = fiorch::WindowRegistry::instance().frames();
    QSet<fiorch::WindowFrame*> bound; // frames already paired with a FrameLayout this call
    ...
    for (const auto& fl : workspace.frames) {
        ...
        fiorch::WindowFrame* target = nullptr;
        bool spawned = false;
        ...
        if (!target) {
            ...
            target = new fiorch::WindowFrame(
                fiorch::WindowFrame::next_window_id(),
                /*parent=*/nullptr,
        ...
        if (variant && spawned) {
            ...
            if (scr_it != variant->frame_screens.constEnd()) {
                if (QScreen* saved_screen =
                        fiorch::MonitorTopology::find_screen_by_stable_id(scr_it.value())) {

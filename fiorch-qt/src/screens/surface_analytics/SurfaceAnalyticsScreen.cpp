...
float SurfaceAnalyticsScreen::spot_for(const QString& sym) const {
    ...
    if (v.isValid()) {
        if (v.canConvert<fiorch::services::QuoteData>()) {
            auto q = v.value<fiorch::services::QuoteData>();
            if (q.price > 0.0)
...
void SurfaceAnalyticsScreen::on_group_symbol_changed(const fiorch::SymbolRef& ref) {
...
fiorch::SymbolRef SurfaceAnalyticsScreen::current_symbol() const {
    if (!control_panel_)
        return {};
    QString s = control_panel_->state().symbol;
    if (s.isEmpty())
        return {};
    return fiorch::SymbolRef::equity(s);
}

...
namespace rep = ::fiorch::report;
using Service = ::fiorch::services::ReportBuilderService;
    ...
    connect(properties_, &PropertiesPanel::config_changed, this,
            [this](int idx, const QString& key, const QString& val) {
                ...
                if (comp_type == "stats_block" && key == "fetch_ticker") {
                    ...
                    fiorch::services::MarketDataService::instance().fetch_info(
                        sym, [self, comp_id, sym](bool ok, fiorch::services::InfoData info) {
                            if (!self)
                                return;
                ...
                if (comp_type == "chart" && key == "fetch_history") {
                    ...
                    auto apply = [self, comp_id, sym](const QVector<fiorch::services::HistoryPoint>& history) {
                        if (!self)
                            return;
                        auto& s = Service::instance();
                        int idx2 = s.index_of(comp_id);
                    ...
                    if (cached.isValid() && cached.canConvert<QVector<fiorch::services::HistoryPoint>>()) {
                        apply(cached.value<QVector<fiorch::services::HistoryPoint>>());
                        return;
                    }

                    hub.subscribe<QVector<fiorch::services::HistoryPoint>>(
                        this, topic,
                        [self, topic, apply](const QVector<fiorch::services::HistoryPoint>& history) {
                            if (!self)
                                return;
                            apply(history);
                            fiorch::datahub::DataHub::instance().unsubscribe(self, topic);
                        });
                ...
                if (comp_type == "market_data" && key == "status" && val == "loading") {
                    ...
                    auto apply = [self, comp_id](const fiorch::services::QuoteData& q) {
                        if (!self)
                            return;
                        auto& s = Service::instance();
                        int idx2 = s.index_of(comp_id);
                    ...
                    QVariant cached = hub.peek(topic);
                    if (cached.isValid() && cached.canConvert<fiorch::services::QuoteData>()) {
                        apply(cached.value<fiorch::services::QuoteData>());
                        return;
                    }
                    hub.subscribe<fiorch::services::QuoteData>(
                        this, topic,
                        [self, topic, apply](const fiorch::services::QuoteData& q) {
                            if (!self)
                                return;
                            apply(q);
                            fiorch::datahub::DataHub::instance().unsubscribe(self, topic);

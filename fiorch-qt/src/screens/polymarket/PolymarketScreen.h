...
namespace fiorch::screens::polymarket {
...
namespace fiorch::services::prediction {
...
class PolymarketScreen : public QWidget, public fiorch::screens::IStatefulScreen {
    Q_OBJECT
  public:
  ...
  private slots:
    ...
    // Browse panel
    void on_market_selected(const fiorch::services::prediction::PredictionMarket& market);
    void on_event_selected(const fiorch::services::prediction::PredictionEvent& event);

    // Detail panel
    void on_interval_changed(const QString& interval);
    void on_outcome_changed(int index);
    void on_related_market_clicked(const fiorch::services::prediction::PredictionMarket& market);

    // Adapter (unified) responses
    void on_markets_ready(const QVector<fiorch::services::prediction::PredictionMarket>& markets);
    void on_events_ready(const QVector<fiorch::services::prediction::PredictionEvent>& events);
    void on_search_results_ready(const QVector<fiorch::services::prediction::PredictionMarket>& markets,
                                 const QVector<fiorch::services::prediction::PredictionEvent>& events);
    void on_tags_ready(const QStringList& tags);
    void on_order_book_ready(const fiorch::services::prediction::PredictionOrderBook& book);
    void on_price_history_ready(const fiorch::services::prediction::PriceHistory& history);
    void on_trades_ready(const QVector<fiorch::services::prediction::PredictionTrade>& trades);
    ...
    void on_ws_orderbook_updated(const QString& asset_id,
                                  const fiorch::services::prediction::PredictionOrderBook& book);
    void on_ws_connection_changed(bool connected);

    // Polymarket-only (PolymarketService) enrichments
    void on_price_summary_received(const fiorch::services::polymarket::PriceSummary& summary);
    void on_top_holders_received(const QVector<fiorch::services::polymarket::TopHolder>& holders);
    void on_comments_received(const QVector<fiorch::services::polymarket::Comment>& comments);
    void on_related_markets_received(const QVector<fiorch::services::polymarket::Market>& markets);
    ...
    void on_kalshi_ws_trade(const fiorch::services::prediction::PredictionTrade& trade);
    void on_kalshi_market_lifecycle(const QString& ticker, const QString& status);
    void on_kalshi_batch_candles(
        const QHash<QString, fiorch::services::prediction::PriceHistory>& histories);
    void on_kalshi_market_detail(const fiorch::services::prediction::PredictionMarket& market);
    void on_kalshi_series_detail(const QString& series_ticker, const QJsonObject& series);

    // Open-orders blotter.
    void on_open_orders_ready(const QVector<fiorch::services::prediction::OpenOrder>& orders);
  ...
  private:
    ...
    void select_market(const fiorch::services::prediction::PredictionMarket& market);
    void subscribe_to_market(const fiorch::services::prediction::PredictionMarket& market);
    void unsubscribe_current();

    bool active_is_polymarket() const;
    fiorch::services::prediction::PredictionExchangeAdapter* active_adapter() const;
    ...
    fiorch::services::prediction::PredictionMarket selected_market_;
    ...
    QHash<QString, fiorch::services::prediction::PriceHistory> batch_candles_cache_;
};

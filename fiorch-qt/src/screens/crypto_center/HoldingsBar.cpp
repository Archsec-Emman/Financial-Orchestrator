...
HoldingsBar::HoldingsBar(QWidget* parent) : QWidget(parent) {
    ...
    connect(&svc, &fiorch::wallet::WalletService::balance_mode_changed, this,
...
void HoldingsBar::on_discount_update(const QVariant& v) {
    if (!v.canConvert<fiorch::wallet::FncptDiscount>()) return;
    const auto d = v.value<fiorch::wallet::FncptDiscount>();
...
void HoldingsBar::resubscribe_prices() {
    ...
    QSet<QString> wanted;
    wanted.insert(QString::fromLatin1(fiorch::wallet::kWrappedSolMint));
    wanted.insert(QString::fromLatin1(fiorch::wallet::kFncptMint));
...
void HoldingsBar::on_price_update(const QString& mint, const QVariant& v) {
    ...
    if (mint == QString::fromLatin1(fiorch::wallet::kFncptMint)) {
...
void HoldingsBar::recompute_total() {
    ...
    // SOL
    const auto sol_mint = QString::fromLatin1(fiorch::wallet::kWrappedSolMint);

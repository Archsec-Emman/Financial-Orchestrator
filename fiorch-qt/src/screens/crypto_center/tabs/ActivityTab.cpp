...
QString kind_label(fiorch::wallet::ParsedActivity::Kind k) {
    using K = fiorch::wallet::ParsedActivity::Kind;
    switch (k) {
...
void ActivityTab::on_activity_update(const QVariant& v) {
    if (v.canConvert<QVector<fiorch::wallet::ParsedActivity>>()) {
        latest_ = v.value<QVector<fiorch::wallet::ParsedActivity>>();
        rebuild_table();
    }
}

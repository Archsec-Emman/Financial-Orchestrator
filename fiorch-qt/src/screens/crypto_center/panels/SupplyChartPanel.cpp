...
void SupplyChartPanel::on_supply_history_update(const QVariant& v) {
    if (!v.canConvert<QVector<fiorch::wallet::SupplyHistoryPoint>>()) return;
    const auto pts = v.value<QVector<fiorch::wallet::SupplyHistoryPoint>>();
    ...
    bool is_mock = false;
    const auto epoch_v = fiorch::datahub::DataHub::instance().peek(
        QStringLiteral("treasury:buyback_epoch"));
    if (epoch_v.canConvert<fiorch::wallet::BuybackEpoch>()) {
        is_mock = epoch_v.value<fiorch::wallet::BuybackEpoch>().is_mock;
    }

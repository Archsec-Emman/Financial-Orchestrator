...
void BuybackBurnPanel::on_epoch_update(const QVariant& v) {
    if (!v.canConvert<fiorch::wallet::BuybackEpoch>()) return;
    const auto e = v.value<fiorch::wallet::BuybackEpoch>();
...
void BuybackBurnPanel::on_burn_total_update(const QVariant& v) {
    if (!v.canConvert<fiorch::wallet::BurnTotal>()) return;
    const auto t = v.value<fiorch::wallet::BurnTotal>();

...
double threshold_ui(const fiorch::wallet::FncptDiscount& d) {
    ...
    auto* save_caption = new QLabel(
        tr("PROJECTED SAVINGS  ·  reference $%1 SKU")
            .arg(QLocale::system().toString(
                fiorch::billing::FeeDiscountConfig::kReferencePriceUsd, 'f', 2)),
        body);
    ...
    hint_ = new QLabel(
        tr("Hold ≥ %1 $FNCPT to qualify for the discount on premium screens, "
           "AI reports, and deep backtests.")
            .arg(QLocale::system().toString(
                static_cast<double>(
                    fiorch::billing::FeeDiscountConfig::kThresholdRaw)
                    / std::pow(10.0, fiorch::billing::FeeDiscountConfig::kThresholdDecimals),
                'f', 0)),
        body);
...
void FeeDiscountPanel::on_discount_update(const QVariant& v) {
    ...
    latest_ = v.value<fiorch::wallet::FncptDiscount>();
    have_discount_ = true;
    update_view();
}

void FeeDiscountPanel::update_view() {
    using fiorch::billing::FeeDiscountConfig;

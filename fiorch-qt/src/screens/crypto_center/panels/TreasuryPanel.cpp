...
void TreasuryPanel::on_reserves_update(const QVariant& v) {
    if (!v.canConvert<fiorch::wallet::TreasuryReserves>()) return;
    const auto r = v.value<fiorch::wallet::TreasuryReserves>();
...
void TreasuryPanel::on_runway_update(const QVariant& v) {
    if (!v.canConvert<fiorch::wallet::TreasuryRunway>()) return;
    const auto w = v.value<fiorch::wallet::TreasuryRunway>();

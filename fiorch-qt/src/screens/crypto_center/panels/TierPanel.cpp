...
void TierPanel::on_wallet_disconnected() {
    ...
    render_state(fiorch::wallet::TierStatus::Tier::Free,
...
void TierPanel::on_tier_update(const QVariant& v) {
    ...
    const auto s = v.value<fiorch::wallet::TierStatus>();
...
void TierPanel::render_state(fiorch::wallet::TierStatus::Tier current,
                              const QString& weight_ui_str,
                              const QString& next_threshold_ui_str,
                              bool is_mock) {
    using Tier = fiorch::wallet::TierStatus::Tier;
    ...
    QString head_text = QStringLiteral("current %1")
                             .arg(fiorch::billing::TierConfig::label_for(current));
    ...
    if (current_pubkey_.isEmpty()) {
        ...
        footer_->setText(tr("All Financial Orchestrator features unlocked."));
        footer_->show();
    } else {
        footer_->hide();
    }

...
void LockPanel::on_wallet_disconnected() {
    fiorch::datahub::DataHub::instance().unsubscribe(this);
    ...
    current_tier_ = fiorch::wallet::TierStatus::Tier::Free;
...
void LockPanel::on_balance_update(const QVariant& v) {
    ...
    const auto bal = v.value<fiorch::wallet::WalletBalance>();
...
void LockPanel::on_vefncpt_update(const QVariant& v) {
    if (!v.canConvert<fiorch::wallet::VeFncptAggregate>()) return;
    const auto agg = v.value<fiorch::wallet::VeFncptAggregate>();
    bool ok = false;
...
void LockPanel::on_revenue_update(const QVariant& v) {
    if (!v.canConvert<fiorch::wallet::TreasuryRevenue>()) return;
    weekly_revenue_usd_ = v.value<fiorch::wallet::TreasuryRevenue>().total_usd;
    recompute_preview();
}
...
void LockPanel::on_tier_update(const QVariant& v) {
    if (!v.canConvert<fiorch::wallet::TierStatus>()) return;
    current_tier_ = v.value<fiorch::wallet::TierStatus>().tier;
    recompute_preview();
}
...
void LockPanel::recompute_preview() {
    ...
    const double mult = fiorch::wallet::StakingService::multiplier_for(d);
    const QString dur_label = fiorch::wallet::StakingService::label_for(d);
    ...
    const auto tier_after = fiorch::billing::TierConfig::tier_from_weight(
        current_user_weight_raw_ + new_weight_raw);
    if (tier_after == current_tier_) {
        tier_preview_->setText(fiorch::billing::TierConfig::label_for(current_tier_));
    } else {
        tier_preview_->setText(QStringLiteral("%1 → %2  (after lock)")
            .arg(fiorch::billing::TierConfig::label_for(current_tier_))
            .arg(fiorch::billing::TierConfig::label_for(tier_after)));
    }
    ...
    if (!fiorch::wallet::StakingService::instance().program_is_configured()) {
...
void LockPanel::on_lock_clicked() {
    ...
    fiorch::wallet::StakingService::instance().build_lock_tx(
        ...
        summary.rows.append({QStringLiteral("DURATION"),
                             fiorch::wallet::StakingService::label_for(d), true});
        const double mult = fiorch::wallet::StakingService::multiplier_for(d);
        ...
        QObject::connect(dlg, &WalletActionConfirmDialog::confirmed, self,
            ...
            fiorch::wallet::WalletService::instance().sign_and_send(
                ...
                [self](Result<QString> sr) {
                    ...
                    fiorch::wallet::WalletService::instance().force_balance_refresh();
                });
        });

...
namespace fiorch::screens::panels {
...
bool is_native_sol(const QString& mint) {
    return mint == QString::fromLatin1(fiorch::wallet::kWrappedSolMint);
}

bool is_fncpt(const QString& mint) {
    return mint == QString::fromLatin1(fiorch::wallet::kFncptMint);
}
...
QString resolve_symbol(const QString& mint) {
    ...
    auto md = fiorch::wallet::TokenMetadataService::instance().lookup(mint);
...
SwapPanel::SwapPanel(QWidget* parent) : QWidget(parent) {
    ...
    sol_holding_.mint = QString::fromLatin1(fiorch::wallet::kWrappedSolMint);
    ...
    from_mint_ = QString::fromLatin1(fiorch::wallet::kWrappedSolMint);
    to_mint_ = QString::fromLatin1(fiorch::wallet::kFncptMint);
    ...
    auto& svc = fiorch::wallet::WalletService::instance();
    connect(&svc, &fiorch::wallet::WalletService::wallet_connected, this,
            &SwapPanel::on_wallet_connected);
    connect(&svc, &fiorch::wallet::WalletService::wallet_disconnected, this,
            &SwapPanel::on_wallet_disconnected);

    auto& hub = fiorch::datahub::DataHub::instance();
    connect(&hub, &fiorch::datahub::DataHub::topic_error, this,
...
void SwapPanel::on_balance_update(const QVariant& v) {
    if (!v.canConvert<fiorch::wallet::WalletBalance>()) return;
    latest_balance_ = v.value<fiorch::wallet::WalletBalance>();
...
void SwapPanel::on_price_update(const QString& mint, const QVariant& v) {
    if (!v.canConvert<fiorch::wallet::TokenPrice>()) return;
    const auto p = v.value<fiorch::wallet::TokenPrice>();
    if (!p.valid) return;
...
void SwapPanel::on_from_changed(int idx) {
    ...
    if (to_mint_ == from_mint_) {
        to_mint_ = is_fncpt(from_mint_)
            ? QString::fromLatin1(fiorch::wallet::kWrappedSolMint)
            : QString::fromLatin1(fiorch::wallet::kFncptMint);
        rebuild_to_combo();
    }
    resubscribe_prices();
    update_balance_label();
...
void SwapPanel::rebuild_from_combo() {
    ...
    from_combo_->addItem(resolve_symbol(QString::fromLatin1(fiorch::wallet::kWrappedSolMint)),
                         QString::fromLatin1(fiorch::wallet::kWrappedSolMint));
    ...
    if (from_combo_->findData(QString::fromLatin1(fiorch::wallet::kFncptMint)) < 0) {
        from_combo_->addItem(resolve_symbol(QString::fromLatin1(fiorch::wallet::kFncptMint)),
                             QString::fromLatin1(fiorch::wallet::kFncptMint));
    }
...
void SwapPanel::rebuild_to_combo() {
    ...
    add(QString::fromLatin1(fiorch::wallet::kFncptMint));
    add(QString::fromLatin1(fiorch::wallet::kWrappedSolMint));
...
const fiorch::wallet::TokenHolding* SwapPanel::holding_for(const QString& mint) const {
...
void SwapPanel::start_status_poll(const QString& sig) {
    auto* rpc = new fiorch::wallet::SolanaRpcClient(this);
    rpc->reload_endpoint();
    ...
    QObject::connect(poll_timer, &QTimer::timeout, this,
        ...
        rpc->get_signature_statuses(QStringList{sig},
            [guard, sig, poll_timer](
                Result<std::vector<fiorch::wallet::SolanaRpcClient::SignatureStatus>> r) {
            if (!guard) return;
            ...
            if (s.confirmation_status == QStringLiteral("confirmed")
                ...
                fiorch::wallet::WalletService::instance()
...
void SwapPanel::on_swap_clicked() {
    ...
    auto* svc = fiorch::wallet::WalletService::instance().swap_service();
    if (!svc) {
    ...
    const auto action = buying_fncpt
                            ? fiorch::wallet::PumpFunSwapService::Action::Buy
                            : fiorch::wallet::PumpFunSwapService::Action::Sell;
    ...
    svc->build_swap(action,
                    ...
                    [self, ui_amount, buying_fncpt, est_out, slip_bps_for_display](
                        Result<fiorch::wallet::PumpFunSwapService::SwapTransaction> r) {
        if (!self) return;
        if (r.is_err()) {
            self->set_busy(false);
        ...
        auto* rpc = new fiorch::wallet::SolanaRpcClient(self);
        rpc->reload_endpoint();
        rpc->simulate_transaction(tx.tx_base64,
            [self, tx, ui_amount, buying_fncpt, est_out, slip_bps_for_display, rpc](
                Result<fiorch::wallet::SolanaRpcClient::SimulationResult> sr) {
            rpc->deleteLater();
            if (!self) return;
            if (sr.is_err()) {
                self->set_busy(false);
            ...
            QObject::connect(dlg, &WalletActionConfirmDialog::confirmed, self,
                ...
                auto* fresh_rpc = new fiorch::wallet::SolanaRpcClient(self);
                fresh_rpc->reload_endpoint();
                fresh_rpc->simulate_transaction(tx_b64,
                    [self, tx_b64, fresh_rpc](
                        Result<fiorch::wallet::SolanaRpcClient::SimulationResult> fr) {
                    ...
                    fiorch::wallet::WalletService::instance().sign_and_send(
                        tx_b64,

...
bool position_is_expired(const fiorch::wallet::LockPosition& p) {
...
void ActiveLocksPanel::on_locks_update(const QVariant& v) {
    if (!v.canConvert<QVector<fiorch::wallet::LockPosition>>()) return;
    latest_ = v.value<QVector<fiorch::wallet::LockPosition>>();
    rebuild_table();
}
...
void ActiveLocksPanel::on_row_context_menu(const QPoint& pos) {
    ...
    if (p.is_mock || !fiorch::wallet::StakingService::instance().program_is_configured()) {

...
static const QList<fiorch::mcp::MarketplaceEntry>& g_catalog = fiorch::mcp::marketplace_catalog();
...
void McpServersScreen::refresh_tools() {
    ...
    for (const auto& t : internal_tools) {
        ...
        // "Financial Orchestrator" is the product name — kept in English for brand consistency.
        auto* si = new QTableWidgetItem(QStringLiteral("Financial Orchestrator"));

...
CrashRecoveryDialog::CrashRecoveryDialog(fiorch::CrashRecovery* recovery,
                                         fiorch::WorkspaceSnapshotRing* ring,
                                         QWidget* parent)
    : QDialog(parent), recovery_(recovery), ring_(ring) {
    setWindowTitle(tr("Financial Orchestrator — Recover Previous Session"));
...
QString CrashRecoveryDialog::display_name(
    const fiorch::WorkspaceSnapshotRing::Entry& e) const {
    if (!e.name.isEmpty())
        return e.name;

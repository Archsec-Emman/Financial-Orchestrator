...
QWidget* ParameterWidgetFactory::create(const ParamDef& param, const QJsonValue& current_value,
    ...
    if (param.type == "string" || param.type == "expression") {
        ...
        combo->setStyleSheet(QString("QComboBox { %1 }"
                                     "QComboBox::drop-down {"
                                     "  background: %2; border: 1px solid %2; width: 18px;"
                                     "}"
        ...
        auto populate = [combo, allowed_exts](const QString& select_path = {}) {
            ...
            auto& svc = fiorch::services::FileManagerService::instance();
        ...
        QObject::connect(import_btn, &QPushButton::clicked, container,
                         [populate, dialog_filter, key, on_change, import_btn]() {
                             ...
                             QString file_id =
                                 fiorch::services::FileManagerService::instance().import_file(path, "workflow_node");
                             if (file_id.isEmpty())
                                 return;

                             // Find the full managed path for the newly imported file
                             auto f = fiorch::services::FileManagerService::instance().find_by_id(file_id);
                             QString full_path = fiorch::services::FileManagerService::instance().full_path(f.name);
        ...
        auto populate_agents = [combo, current_value]() {
            ...
            const auto agents = fiorch::services::AgentService::instance().cached_agents();
        ...
        auto trigger_discovery = [populate_agents, combo]() {
            auto& svc = fiorch::services::AgentService::instance();
            QObject::connect(&svc, &fiorch::services::AgentService::agents_discovered, combo,
                             [populate_agents, combo](const QVector<fiorch::services::AgentInfo>&,
                                                      const QVector<fiorch::services::AgentCategory>&) {
                                 populate_agents();
                                 QObject::disconnect(&fiorch::services::AgentService::instance(),
                                                     &fiorch::services::AgentService::agents_discovered, combo,
                                                     nullptr);
                             });
            svc.discover_agents();
        };
        ...
        QString saved = current_value.toString();
        auto res = fiorch::LlmProfileRepository::instance().list_profiles();
        if (res.is_ok()) {
            ...
            for (const auto& t : tools) {
                if (t.server_name != last_server) {
                    ...
                    if (model) {
                        ...
                        sep->setData(QColor(ui::colors::TEXT_TERTIARY.get()), Qt::ForegroundRole);
                    }

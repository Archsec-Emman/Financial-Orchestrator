...
using namespace fiorch::services::geo;
...
void ConflictMonitorPanel::build_ui() {
    ...
    // ── World Map ───────────────────────────────────────────────────────────
    map_widget_ = new fiorch::ui::WorldMapWidget(left_splitter);
    ...
    connect(map_widget_, &fiorch::ui::WorldMapWidget::pin_clicked, this, [this](int event_id) {
        if (!events_table_)
            return;

...
void ChatModeScreen::wire_signals() {
    ...
    connect(message_panel_, &ChatMessagePanel::draft_changed, this,
            [this]() { fiorch::ScreenStateManager::instance().notify_changed(this); });
    ...
    connect(session_panel_, &ChatSessionPanel::delete_session_requested, this, &ChatModeScreen::on_delete_session);
    connect(session_panel_, &ChatSessionPanel::rename_session_requested, this, &ChatModeScreen::on_rename_session);
    connect(session_panel_, &ChatSessionPanel::exit_chat_mode_requested, this, &ChatModeScreen::exit_requested);

    // Message panel -> screen
    connect(message_panel_, &ChatMessagePanel::send_requested, this, &ChatModeScreen::on_send_requested);

    // SSE service -> message panel
    connect(&svc, &ChatModeService::stream_session_meta, message_panel_, &ChatMessagePanel::on_stream_session_meta);
    connect(&svc, &ChatModeService::stream_text_delta, message_panel_, &ChatMessagePanel::on_stream_text_delta);
    connect(&svc, &ChatModeService::stream_tool_end, message_panel_, &ChatMessagePanel::on_stream_tool_end);
    connect(&svc, &ChatModeService::stream_step_start, message_panel_, &ChatMessagePanel::on_stream_step_start);
    connect(&svc, &ChatModeService::stream_step_finish, message_panel_, &ChatMessagePanel::on_stream_step_finish);
    connect(&svc, &ChatModeService::stream_thinking, message_panel_, &ChatMessagePanel::on_stream_thinking);
    ...
    connect(&svc, &ChatModeService::stream_finish, this, [this](int) {
        session_panel_->refresh_sessions();
        QPointer<ChatModeScreen> s = this;
        QTimer::singleShot(1500, this, [s]() {
            if (!s)
                return;
            ChatModeService::instance().get_credits([s](bool ok, int credits, QString) {
                if (s && ok)
                    s->message_panel_->set_credits(credits);
            });
        });
    });

    // Tool bridge -> message panel (shows tool count in header)
    connect(tool_bridge_, &TerminalToolBridge::tools_registered, message_panel_,
            &ChatMessagePanel::on_tools_registered);
}
...
void ChatModeScreen::showEvent(QShowEvent* e) {
    ...
    // Load stats
    ChatModeService::instance().get_stats([self](bool ok, ChatStats stats, QString) {
        if (!self)
            return;
        if (ok)
            self->session_panel_->update_stats(stats);
    });

    // Restore most recent session
    if (active_session_uuid_.isEmpty()) {
        LOG_INFO("ChatModeScreen", "No active session -- loading most recent");
        ChatModeService::instance().list_sessions([self](bool ok, QVector<ChatSession> sessions, QString err) {
            if (!self)
                return;
            if (!ok) {
                LOG_WARN("ChatModeScreen", "List sessions failed: " + err);
                return;
            }
            if (sessions.isEmpty())
                return;
...
void ChatModeScreen::on_session_selected(const QString& uuid) {
    ...
    fiorch::ScreenStateManager::instance().notify_changed(this);
...
void ChatModeScreen::on_delete_session(const QString& uuid) {
    QPointer<ChatModeScreen> self = this;
    ChatModeService::instance().delete_session(uuid, [self, uuid](bool ok, QString err) {
        if (!self)
            return;
        if (!ok) {
            LOG_WARN("ChatModeScreen", "Delete failed: " + err);
            return;
        }
        ...
        ChatModeService::instance().list_sessions([self](bool list_ok, QVector<ChatSession> sessions, QString) {
            if (!self)
                return;
...
void ChatModeScreen::on_rename_session(const QString& uuid, const QString& title) {
    QPointer<ChatModeScreen> self = this;
    ChatModeService::instance().rename_session(uuid, title, [self](bool ok, QString err) {
        if (!self)
            return;
        if (!ok)
            LOG_WARN("ChatModeScreen", "Rename failed: " + err);
        self->session_panel_->refresh_sessions();
    });
}

...
void ForumFeedPanel::build_toolbar() {
    ...
    chips_scroll->setStyleSheet(QString("QScrollArea{border:none;background:transparent;"
                                        "border-top:1px solid %1;}")
                                    .arg(ui::colors::BORDER_DIM()));
    chips_scroll->setWidgetResizable(false);

    chips_container_ = new QWidget(this);
    ...
    for (const auto& cat : categories_) {
        ...
        chip->setStyleSheet(active ? QString("QPushButton{background:%1;color:%2;"
                                   ...
                                   : QString("QPushButton{background:transparent;color:%1;"
                                         ...
                                         .arg(ui::colors::TEXT_SECONDARY(), ui::colors::BORDER_DIM(), M(10),
                                              ui::colors::TEXT_PRIMARY(), ui::colors::BORDER_MED()));
        connect(chip, &QPushButton::clicked, this,
                [this, cat]() { emit category_clicked(cat.id, cat.name, cat.color); });
...
void ForumFeedPanel::rebuild_posts() {
    ...
    for (int i = 0; i < page_.posts.size(); ++i) {
        ...
        card->setStyleSheet(active ? QString("background:rgba(217,119,6,0.04);"
        ...
        auto* up_btn = new QPushButton(QString("▲ %1").arg(fmt_n(p.likes)));
        ...
        up_btn->setStyleSheet(voted_up ? QString("QPushButton{background:rgba(217,119,6,0.12);"
        ...
        auto mk_eng = [&](const QString& icon, const QString& val, const QString& col) {
            ...
            lbl->setStyleSheet(QString("color:%1;font-size:10px;background:transparent;%2").arg(col, M(10)));
            return lbl;
        };
    ...
    if (page_.page < page_.pages) {
        ...
        more->setStyleSheet(QString("QPushButton{background:%1;color:%2;"
                                ...
                                .arg(ui::colors::BG_SURFACE(), ui::colors::TEXT_TERTIARY(), ui::colors::BORDER_DIM(),
                                     M(12), ui::colors::AMBER(), ui::colors::BORDER_MED()));
        int np = page_.page + 1;
        connect(more, &QPushButton::clicked, this, [this, np]() { emit load_more_requested(np); });
...
void ForumFeedPanel::show_skeleton() {
    ...
    for (int i = 0; i < 6; ++i) {
        ...
        r1h->setContentsMargins(0, 0, 0, 0);
        r1h->setSpacing(10);
        auto* av_skel = new QWidget(this);
        av_skel->setFixedSize(28, 28);
        av_skel->setStyleSheet(QString("background:%1;border-radius:14px;").arg(ui::colors::BORDER_DIM()));
        auto* name_skel = new QWidget(this);
        name_skel->setFixedSize(100 + i * 15, 10);
        name_skel->setStyleSheet(QString("background:%1;border-radius:5px;").arg(ui::colors::BORDER_DIM()));
        ...
        // Title bar
        auto* title_skel = new QWidget(this);
        title_skel->setFixedHeight(12);
        title_skel->setMaximumWidth(300 + i * 30);
        title_skel->setStyleSheet(QString("background:qlineargradient(x1:0,y1:0,x2:1,y2:0,"
        ...
        // Body bar
        auto* body_skel = new QWidget(this);
        body_skel->setFixedHeight(8);
        body_skel->setMaximumWidth(200 + i * 20);
        body_skel->setStyleSheet(QString("background:%1;border-radius:4px;").arg(ui::colors::BORDER_DIM()));
...
void ForumFeedPanel::pulse_skeleton() {
    ...
    for (int i = 0; i < vl->count(); ++i) {
        ...
        for (int j = 0; j < cvl->count(); ++j) {
            ...
            if (bi && bi->widget() && bi->widget()->maximumWidth() < 2000 && bi->widget()->height() >= 8 &&
                bi->widget()->height() <= 14)

...
void GovDataAustraliaPanel::build_ui() {
    ...
    configure_table(agencies_table_);
...
QJsonArray GovDataAustraliaPanel::unwrap_datasets(const services::GovDataResult& result, int& out_total) {
    out_total = 0;
...
void GovDataAustraliaPanel::on_result(const QString& request_id, const services::GovDataResult& result) {
    // Only handle request IDs that belong to this panel
    if (!request_id.startsWith("ausgov_"))
        return;
...
void GovDataAustraliaPanel::populate_datasets(const QJsonArray& json, int total_count) {
    ...
    for (int i = 0; i < json.size(); ++i) {
        ...
        if (license.isEmpty())
            license = obj["license_title"].toString();
        if (license.isEmpty())
            license = "—";
        ...
        if (modified.isEmpty())
            modified = obj["metadata_modified"].toString();
...
void GovDataAustraliaPanel::on_dataset_doubleclicked(int row, int /*col*/) {
    ...
    if (dataset_idx < 0 || dataset_idx >= current_datasets_.size())
        return;

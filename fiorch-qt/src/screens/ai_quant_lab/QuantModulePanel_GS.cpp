...
using namespace fiorch::services::quant;
using namespace fiorch::screens::quant_styles;
using namespace fiorch::screens::quant_common;
using namespace fiorch::screens::quant_gs_helpers;
...
void QuantModulePanel::display_gs_result(const QString& command, const QJsonObject& payload) {
    ...
    if (d.contains("n_observations"))
        header_text += tr("  |  %1 OBS").arg(d.value("n_observations").toInt());
    else if (d.contains("n_scenarios"))
        header_text += tr("  |  %1 SCENARIOS").arg(d.value("n_scenarios").toInt());
    ...
    if (command == "risk_metrics") {
        ...
        QList<QWidget*> vol = {
            gs_make_card(tr("VOLATILITY (ANN)"), gs_fmt_pct(d.value("volatility_annualized").toDouble()), this),
            gs_make_card(tr("DAILY RISK"), gs_fmt_pct(d.value("daily_risk").toDouble(), 3), this),
            gs_make_card(tr("DOWNSIDE RISK"), gs_fmt_pct(d.value("downside_risk").toDouble()), this,
                         ui::colors::WARNING()),
            gs_make_card(tr("MAX DRAWDOWN"), gs_fmt_pct(mdd), this,
                         mdd <= -0.20 ? ui::colors::NEGATIVE() : ui::colors::WARNING()),
        };
        results_layout_->addWidget(gs_card_row(vol, this));

        QList<QWidget*> var_cards = {
            gs_make_card(tr("VaR 95%"), gs_fmt_pct(d.value("var_95").toDouble(), 3), this, ui::colors::WARNING()),
            gs_make_card(tr("VaR 99%"), gs_fmt_pct(d.value("var_99").toDouble(), 3), this, ui::colors::NEGATIVE()),
            gs_make_card(tr("DD LENGTH"), tr("%1 days").arg(d.value("max_drawdown_length").toInt()), this),
            gs_make_card(tr("RECOVERY"), tr("%1 days").arg(d.value("max_recovery_period").toInt()), this),
        };
    ...
    if (command == "portfolio_analytics") {
        ...
        QList<QWidget*> top = {
            gs_make_card(tr("ALPHA"), gs_fmt_pct(alpha, 2), this, gs_pos_neg_color(alpha)),
            gs_make_card(tr("SHARPE"), gs_fmt_num(sharpe, 3), this, gs_pos_neg_color(sharpe)),
            gs_make_card(tr("SORTINO"), gs_fmt_num(d.value("sortino_ratio").toDouble(), 3), this),
            gs_make_card(tr("INFO RATIO"), gs_fmt_num(info, 3), this, gs_pos_neg_color(info)),
        };
        results_layout_->addWidget(gs_card_row(top, this));

        QList<QWidget*> vsBench = {
            gs_make_card(tr("TRACKING ERROR"), gs_fmt_pct(te), this),
            gs_make_card(tr("R²"), gs_fmt_num(r2, 4), this),
            gs_make_card(tr("HIT RATE"), QString::number(hit, 'f', 1) + "%", this,
                         hit > 50 ? ui::colors::POSITIVE() : ui::colors::WARNING()),
            gs_make_card(tr("MAX DRAWDOWN"), gs_fmt_pct(d.value("max_drawdown").toDouble()), this,
                         ui::colors::NEGATIVE()),
        };
        results_layout_->addWidget(gs_card_row(vsBench, this));

        QList<QWidget*> capture = {
            gs_make_card(tr("UP CAPTURE"), QString::number(d.value("up_capture").toDouble(), 'f', 1) + "%", this),
            gs_make_card(tr("DOWN CAPTURE"), QString::number(d.value("down_capture").toDouble(), 'f', 1) + "%", this),
            gs_make_card(tr("CAPTURE RATIO"), gs_fmt_num(d.value("capture_ratio").toDouble(), 3), this),
            gs_make_card(tr("ANNUAL RISK"), gs_fmt_pct(d.value("annual_risk").toDouble()), this),
        };
        results_layout_->addWidget(gs_card_row(capture, this));

        QList<QWidget*> jensen = {
            gs_make_card(tr("JENSEN ALPHA"), gs_fmt_pct(d.value("jensen_alpha").toDouble()), this,
                         gs_pos_neg_color(d.value("jensen_alpha").toDouble())),
            gs_make_card(tr("JENSEN BULL"), gs_fmt_pct(d.value("jensen_alpha_bull").toDouble()), this),
            gs_make_card(tr("JENSEN BEAR"), gs_fmt_pct(d.value("jensen_alpha_bear").toDouble()), this),
            gs_make_card(tr("TREYNOR"), gs_fmt_num(d.value("treynor_measure").toDouble(), 3), this),
        };
        results_layout_->addWidget(gs_card_row(jensen, this));

        QList<QWidget*> moments = {
            gs_make_card(tr("SKEWNESS"), gs_fmt_num(d.value("skewness").toDouble(), 3), this),
            gs_make_card(tr("KURTOSIS"), gs_fmt_num(d.value("kurtosis").toDouble(), 3), this),
            gs_make_card(tr("CALMAR"), gs_fmt_num(d.value("calmar_ratio").toDouble(), 3), this),
            gs_make_card(tr("MODIGLIANI"), gs_fmt_pct(d.value("modigliani_ratio").toDouble()), this),
        };
    ...
    if (command == "greeks") {
        ...
        const QString moneyness_label = moneyness > 1.02 ? tr("ITM") : moneyness < 0.98 ? tr("OTM") : tr("ATM");
        ...
        QList<QWidget*> first = {
            gs_make_card(tr("DELTA"), gs_fmt_num(g.value("delta").toDouble(), 4), this,
        ...
        QList<QWidget*> second = {
            gs_make_card(tr("RHO"), gs_fmt_num(g.value("rho").toDouble(), 4), this),
            gs_make_card(tr("VANNA"), gs_fmt_num(g.value("vanna").toDouble(), 4), this),
            gs_make_card(tr("VOLGA"), gs_fmt_num(g.value("volga").toDouble(), 4), this),
            gs_make_card(tr("CHARM"), gs_fmt_num(g.value("charm").toDouble(), 4), this),
        };
        results_layout_->addWidget(gs_card_row(second, this));

        QList<QWidget*> higher = {
            gs_make_card(tr("SPEED"), gs_fmt_num(g.value("speed").toDouble(), 6), this),
            gs_make_card(tr("ZOMMA"), gs_fmt_num(g.value("zomma").toDouble(), 6), this),
            gs_make_card(tr("COLOR"), gs_fmt_num(g.value("color").toDouble(), 6), this),
            gs_make_card(tr("ULTIMA"), gs_fmt_num(g.value("ultima").toDouble(), 6), this),
        };
    ...
    if (command == "var_analysis") {
        ...
        QList<QWidget*> amounts = {
            gs_make_card(tr("PARAMETRIC VaR"), gs_fmt_money(parm.value("var_amount").toDouble()), this,
                         var_color(parm.value("var_amount").toDouble())),
            gs_make_card(tr("HISTORICAL VaR"), gs_fmt_money(hist.value("var_amount").toDouble()), this,
                         var_color(hist.value("var_amount").toDouble())),
            gs_make_card(tr("MONTE CARLO VaR"), gs_fmt_money(mc.value("var_amount").toDouble()), this,
                         var_color(mc.value("var_amount").toDouble())),
            gs_make_card(tr("CVaR (EXP. SHORT.)"), gs_fmt_money(cv.value("var_amount").toDouble()), this,
                         ui::colors::NEGATIVE()),
        };
        ...
        table->setHorizontalHeaderLabels({tr("Method"), tr("VaR Amount"), tr("VaR %"), tr("Notes")});
        ...
        const QList<QPair<QString, QJsonObject>> rows = {
            {tr("Parametric (Variance-Covariance)"), parm},
            {tr("Historical Simulation"), hist},
            {tr("Monte Carlo Simulation"), mc},
            {tr("CVaR / Expected Shortfall"), cv},
        };
    ...
    if (command == "stress_test") {
        ...
        QList<QWidget*> summary_cards = {
            ...
            gs_make_card(tr("WORST CASE %"), QString("%1%").arg(worst_pnl / pos * 100, 0, 'f', 2), this,
                         ui::colors::NEGATIVE()),
            gs_make_card(tr("BEST CASE PnL"), gs_fmt_signed_money(best_pnl), this,
                         best_pnl > 0 ? ui::colors::POSITIVE() : ui::colors::TEXT_PRIMARY()),
            gs_make_card(tr("SCENARIOS RUN"), QString::number(scenarios.size()), this),
        };
        ...
        table->setHorizontalHeaderLabels({tr("Scenario"), tr("PnL"), tr("PnL %"), tr("Shocked Value")});
        table->verticalHeader()->setVisible(false);
        table->setEditTriggers(QAbstractItemView::NoEditTriggers);
    ...
    if (command == "backtest") {
        ...
        QList<QWidget*> top = {
            gs_make_card(tr("FINAL VALUE"), gs_fmt_money(final_val), this, gs_pos_neg_color(final_val - init_cap)),
            gs_make_card(tr("TOTAL RETURN"), gs_fmt_pct(total_ret), this, gs_pos_neg_color(total_ret)),
        ...
        QList<QWidget*> risk_row = {
            gs_make_card(tr("VOLATILITY"), gs_fmt_pct(m.value("volatility").toDouble()), this),
            gs_make_card(tr("MAX DRAWDOWN"), gs_fmt_pct(mdd), this,
                         mdd <= -0.20 ? ui::colors::NEGATIVE() : ui::colors::WARNING()),
            gs_make_card(tr("BEST DAY"), gs_fmt_pct(m.value("best_day").toDouble()), this, ui::colors::POSITIVE()),
            gs_make_card(tr("WORST DAY"), gs_fmt_pct(m.value("worst_day").toDouble()), this, ui::colors::NEGATIVE()),
        };
        results_layout_->addWidget(gs_card_row(risk_row, this));

        QList<QWidget*> activity = {
            gs_make_card(tr("TRADES"), QString::number(trades), this),
            gs_make_card(tr("WIN RATE"), QString::number(m.value("win_rate_pct").toDouble(), 'f', 1) + "%", this,
                         m.value("win_rate_pct").toDouble() > 50 ? ui::colors::POSITIVE() : ui::colors::WARNING()),
            gs_make_card(tr("DAYS"), QString::number(m.value("n_days").toInt()), this),
            gs_make_card(tr("PROFIT"), gs_fmt_signed_money(final_val - init_cap), this,
        ...
        if (curve.size() >= 2) {
            ...
            eq_table->setHorizontalHeaderLabels({tr("Date"), tr("Portfolio Value"), tr("Return")});
    ...
    if (command == "statistics") {
        QList<QWidget*> central = {
            gs_make_card(tr("MEAN"), gs_fmt_num(d.value("mean").toDouble(), 4), this),
            gs_make_card(tr("MEDIAN"), gs_fmt_num(d.value("median").toDouble(), 4), this),
            gs_make_card(tr("STD DEV"), gs_fmt_num(d.value("std").toDouble(), 4), this),
            gs_make_card(tr("VARIANCE"), gs_fmt_num(d.value("variance").toDouble(), 4), this),
        };
        results_layout_->addWidget(gs_card_row(central, this));

        QList<QWidget*> spread = {
            gs_make_card(tr("MIN"), gs_fmt_num(d.value("min").toDouble(), 4), this, ui::colors::NEGATIVE()),
            gs_make_card(tr("MAX"), gs_fmt_num(d.value("max").toDouble(), 4), this, ui::colors::POSITIVE()),
            gs_make_card(tr("RANGE"), gs_fmt_num(d.value("range").toDouble(), 4), this),
            gs_make_card(tr("SUM"), gs_fmt_num(d.value("sum").toDouble(), 2), this),
        };
        ...
        QList<QWidget*> shape = {
            ...
            gs_make_card(tr("KURTOSIS"), gs_fmt_num(d.value("kurtosis").toDouble(), 3), this),
            gs_make_card(tr("SEMI-VAR"), gs_fmt_num(d.value("semi_variance").toDouble(), 4), this),
            gs_make_card(tr("REALIZED VAR"), gs_fmt_num(d.value("realized_variance").toDouble(), 2), this),
        };
        ...
        if (!pcts.isEmpty()) {
            ...
            table->setHorizontalHeaderLabels({tr("Percentile"), tr("Value")});
            table->verticalHeader()->setVisible(false);
            table->setEditTriggers(QAbstractItemView::NoEditTriggers);
            table->setSelectionMode(QAbstractItemView::NoSelection);
            table->horizontalHeader()->setSectionResizeMode(QHeaderView::Stretch);

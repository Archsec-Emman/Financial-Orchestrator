...
void QuantModulePanel::display_statsmodels_result(const QString& command, const QJsonObject& payload) {
    ...
    if (command == "check_status") {
        ...
        QList<QWidget*> deps = {
            bool_card(tr("STATSMODELS"), d.value("statsmodels").toBool()),
    ...
    if (command == "ols") {
        ...
        auto* hdr = new QLabel(tr("OLS  |  %1 OBS  |  %2 FEATURES%3")
                                   ...
                                   .arg(d.value("has_constant").toBool() ? tr("  +  CONSTANT") : QString()));
            ...
            gs_make_card(tr("R²"), gs_fmt_num(r2, 4), this,
                         r2 >= 0.7 ? ui::colors::POSITIVE()
                                  : r2 >= 0.3 ? ui::colors::WARNING()
                                              : ui::colors::NEGATIVE()),
            gs_make_card(tr("ADJ R²"), gs_fmt_num(adj, 4), this),
            gs_make_card(tr("F STATISTIC"), gs_fmt_num(f_stat, 2), this),
            gs_make_card(tr("F p-VALUE"), gs_fmt_num(f_p, 4), this,
                         f_p < 0.05 ? ui::colors::POSITIVE() : ui::colors::WARNING()),
        };
        results_layout_->addWidget(gs_card_row(fit, this));

        QList<QWidget*> ic = {
            gs_make_card(tr("AIC"), gs_fmt_num(d.value("aic").toDouble(), 2), this),
            gs_make_card(tr("BIC"), gs_fmt_num(d.value("bic").toDouble(), 2), this),
            gs_make_card(tr("LOG-LIK"), gs_fmt_num(d.value("log_likelihood").toDouble(), 2), this),
            gs_make_card(tr("RESID SE"), gs_fmt_num(d.value("residual_std_error").toDouble(), 4), this),
        };
        ...
        const QString dw_col = (dw >= 1.5 && dw <= 2.5) ? ui::colors::POSITIVE()
                                                         : ui::colors::WARNING();
        QList<QWidget*> diag = {
            gs_make_card(tr("DURBIN-WATSON"), gs_fmt_num(dw, 3), this, dw_col),
            gs_make_card(tr("AUTOCORR."), d.value("dw_interpretation").toString().toUpper(), this, dw_col),
            gs_make_card(tr("BREUSCH-PAGAN p"), gs_fmt_num(bp_p, 4), this),
            gs_make_card(tr("HOMOSCEDASTIC"), d.value("homoscedastic_5pct").toBool() ? tr("YES") : tr("NO"), this,
        ...
        if (!coefs.isEmpty()) {
            ...
            table->setHorizontalHeaderLabels({tr("Term"), tr("Estimate"), tr("Std. Error"), tr("t"), tr("p"), tr("95% CI Lower"), tr("95% CI Upper")});
    ...
    if (command == "arima") {
        ...
        QList<QWidget*> fit = {
            ...
            gs_make_card(tr("HQIC"), gs_fmt_num(d.value("hqic").toDouble(), 2), this),
            gs_make_card(tr("IN-SAMPLE R²"), gs_fmt_num(r2, 4), this,
                         r2 >= 0.7 ? ui::colors::POSITIVE()
                                  : r2 >= 0.3 ? ui::colors::WARNING()
                                              : ui::colors::NEGATIVE()),
        };
        ...
        QList<QWidget*> resid = {
            gs_make_card(tr("LB(10) p-VALUE"), gs_fmt_num(lb_p, 4), this,
                         whitenoise ? ui::colors::POSITIVE() : ui::colors::WARNING()),
            gs_make_card(tr("RESID WHITE NOISE"), whitenoise ? tr("YES") : tr("NO"), this,
                         whitenoise ? ui::colors::POSITIVE() : ui::colors::WARNING()),
            gs_make_card(tr("RESID MEAN"), gs_fmt_num(d.value("residual_mean").toDouble(), 4), this),
            gs_make_card(tr("RESID σ"), gs_fmt_num(d.value("residual_std").toDouble(), 4), this),
        };
        ...
        QList<QWidget*> proj = {
            ...
            gs_make_card(tr("HORIZON Δ"), gs_fmt_num(horizon_drift, 4), this,
                         gs_pos_neg_color(horizon_drift)),
        };
        ...
        if (!forecast.isEmpty()) {
            ...
            table->setHorizontalHeaderLabels({tr("Step"), tr("Lower 95%"), tr("Point"), tr("Upper 95%")});
            table->verticalHeader()->setVisible(false);
            table->setEditTriggers(QAbstractItemView::NoEditTriggers);
    ...
    if (command == "stationarity_tests") {
        ...
        QList<QWidget*> adf_row = {
            gs_make_card(tr("STATISTIC"), gs_fmt_num(adf.value("statistic").toDouble(), 4), this),
            gs_make_card(tr("p-VALUE"), gs_fmt_num(adf.value("p_value").toDouble(), 4), this,
                         adf.value("stationary_5pct").toBool() ? ui::colors::POSITIVE()
                                                                : ui::colors::WARNING()),
            gs_make_card(tr("VERDICT @ 5%"), adf.value("stationary_5pct").toBool() ? tr("STATIONARY")
                                                                                  : tr("NON-STATIONARY"),
                         this, adf.value("stationary_5pct").toBool() ? ui::colors::POSITIVE()
                                                                       : ui::colors::NEGATIVE()),
            gs_make_card(tr("LAGS USED"), QString::number(adf.value("lags_used").toInt()), this),
        };
        results_layout_->addWidget(gs_card_row(adf_row, this));
        QList<QWidget*> adf_crit = {
            gs_make_card(tr("ADF CRIT 1%"), gs_fmt_num(adf.value("critical_1pct").toDouble(), 4), this),
            gs_make_card(tr("ADF CRIT 5%"), gs_fmt_num(adf.value("critical_5pct").toDouble(), 4), this),
            gs_make_card(tr("ADF CRIT 10%"), gs_fmt_num(adf.value("critical_10pct").toDouble(), 4), this),
            gs_make_card(tr("REGRESSION"), adf.value("regression").toString().toUpper(), this),
        };
        ...
        QList<QWidget*> kp_row = {
            gs_make_card(tr("STATISTIC"), gs_fmt_num(kp.value("statistic").toDouble(), 4), this),
            gs_make_card(tr("p-VALUE"), gs_fmt_num(kp.value("p_value").toDouble(), 4), this,
                         kp.value("stationary_5pct").toBool() ? ui::colors::POSITIVE()
                                                               : ui::colors::WARNING()),
            gs_make_card(tr("VERDICT @ 5%"), kp.value("stationary_5pct").toBool() ? tr("STATIONARY")
                                                                                : tr("NON-STATIONARY"),
                         this, kp.value("stationary_5pct").toBool() ? ui::colors::POSITIVE()
                                                                      : ui::colors::NEGATIVE()),
            gs_make_card(tr("LAGS USED"), QString::number(kp.value("lags_used").toInt()), this),
        };
        results_layout_->addWidget(gs_card_row(kp_row, this));
        QList<QWidget*> kp_crit = {
            gs_make_card(tr("KPSS CRIT 1%"), gs_fmt_num(kp.value("critical_1pct").toDouble(), 4), this),
            gs_make_card(tr("KPSS CRIT 5%"), gs_fmt_num(kp.value("critical_5pct").toDouble(), 4), this),
            gs_make_card(tr("KPSS CRIT 10%"), gs_fmt_num(kp.value("critical_10pct").toDouble(), 4), this),
            gs_make_card(tr("REGRESSION"), kp.value("regression").toString().toUpper(), this),
        };
    ...
    if (command == "acf_pacf") {
        ...
        QList<QWidget*> sum = {
            gs_make_card(tr("SIG. ACF LAGS"), QString::number(d.value("n_significant_acf_lags").toInt()), this),
            gs_make_card(tr("SIG. PACF LAGS"), QString::number(d.value("n_significant_pacf_lags").toInt()), this),
            gs_make_card(tr("SUGGESTED p"), QString::number(p_sug), this, ui::colors::INFO()),
            gs_make_card(tr("SUGGESTED q"), QString::number(q_sug), this, ui::colors::INFO()),
        };
        ...
        if (!lags.isEmpty()) {
            ...
            table->setHorizontalHeaderLabels({tr("Lag"), tr("ACF"), tr("ACF Sig."), tr("PACF"), tr("PACF Sig.")});
            table->verticalHeader()->setVisible(false);
            table->setEditTriggers(QAbstractItemView::NoEditTriggers);
            table->setSelectionMode(QAbstractItemView::NoSelection);
            table->horizontalHeader()->setSectionResizeMode(QHeaderView::Stretch);
    ...
    if (command == "granger_causality") {
        ...
        QList<QWidget*> sum = {
            gs_make_card(tr("ANY CAUSAL LAG"), any_causal ? tr("YES") : tr("NO"), this,
                         any_causal ? ui::colors::POSITIVE() : ui::colors::TEXT_PRIMARY()),
            gs_make_card(tr("CAUSAL LAGS"), causal_strs.isEmpty() ? QStringLiteral("—") : causal_strs.join(", "), this,
                         any_causal ? ui::colors::POSITIVE() : ui::colors::TEXT_TERTIARY()),
            gs_make_card(tr("LAGS TESTED"), QString::number(d.value("max_lag").toInt()), this),
            gs_make_card(tr("OBS"), QString::number(d.value("n_observations").toInt()), this),
        };
        results_layout_->addWidget(gs_card_row(sum, this));
        ...
        if (!tests.isEmpty()) {
            ...
            table->setHorizontalHeaderLabels({tr("Lag"), tr("F-stat"), tr("F p-value"), tr("χ² stat"), tr("χ² p-value"), tr("x ⇒ y @ 5%")});
            table->verticalHeader()->setVisible(false);
            table->setEditTriggers(QAbstractItemView::NoEditTriggers);
            table->setSelectionMode(QAbstractItemView::NoSelection);
            table->horizontalHeader()->setSectionResizeMode(QHeaderView::Stretch);
    ...
    if (command == "descriptive") {
        ...
        QList<QWidget*> spread = {
            ...
            gs_make_card(tr("IQR"), gs_fmt_num(d.value("iqr").toDouble(), 4), this),
        };
        results_layout_->addWidget(gs_card_row(spread, this));

        QList<QWidget*> qrt = {
            gs_make_card(tr("p25"), gs_fmt_num(d.value("p25").toDouble(), 4), this),
            gs_make_card(tr("p50"), gs_fmt_num(d.value("p50").toDouble(), 4), this),
            gs_make_card(tr("p75"), gs_fmt_num(d.value("p75").toDouble(), 4), this),
            gs_make_card(tr("CV"),
        ...
        QList<QWidget*> shape = {
            ...
            gs_make_card(tr("KURTOSIS (excess)"), gs_fmt_num(d.value("kurtosis_excess").toDouble(), 3), this),
        ...
        QList<QWidget*> norm = {
            gs_make_card(tr("JARQUE-BERA STAT"), gs_fmt_num(d.value("jarque_bera_stat").toDouble(), 3), this),
            gs_make_card(tr("JARQUE-BERA p"), gs_fmt_num(d.value("jarque_bera_p").toDouble(), 4), this,
                         jb_normal.isBool()
                             ? (jb_normal.toBool() ? ui::colors::POSITIVE() : ui::colors::WARNING())
                             : ui::colors::TEXT_PRIMARY()),
            gs_make_card(tr("SHAPIRO-WILK p"), gs_fmt_num(d.value("shapiro_wilk_p").toDouble(), 4), this,
                         sw_normal.isBool()
                             ? (sw_normal.toBool() ? ui::colors::POSITIVE() : ui::colors::WARNING())
                             : ui::colors::TEXT_PRIMARY()),
            gs_make_card(tr("NORMAL (BOTH)"),

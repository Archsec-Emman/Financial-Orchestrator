...
namespace fiorch::screens::quant_advanced_helpers {
...
inline QString verdict_color_for(const QString& v) {
    ...
    if (u == "STRONG" || u == "GOOD" || u == "EXCELLENT" || u == "CLEAN" || u == "NORMAL" || u == "BUY")
        return fiorch::ui::colors::POSITIVE();
    if (u == "MODERATE" || u == "MAINTAIN" || u == "FAIR" || u == "MIXED")
        return fiorch::ui::colors::TEXT_PRIMARY();
    if (u == "WEAK" || u == "WARNING" || u == "ELEVATED" || u == "NEUTRAL")
        return fiorch::ui::colors::WARNING();
    if (u == "TOXIC" || u == "POOR" || u == "AVOID" || u == "SELL")
        return fiorch::ui::colors::NEGATIVE();
    return fiorch::ui::colors::TEXT_PRIMARY();
}
...
inline QTableWidget* build_weights_table(const QJsonObject& weights,
    ...
    table->setStyleSheet(fiorch::screens::quant_styles::table_ss());
    ...
    for (int r = 0; r < rows.size(); ++r) {
        ...
        if (w > 0.001) wi->setForeground(QColor(fiorch::ui::colors::POSITIVE()));
        else if (w < -0.001) wi->setForeground(QColor(fiorch::ui::colors::NEGATIVE()));

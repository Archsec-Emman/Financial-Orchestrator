...
void ChatBubbleController::on_frame_added(fiorch::WindowFrame* w) {
    if (!w) return;
    tracked_frames_.insert(w, QPointer<fiorch::WindowFrame>(w));
}

void ChatBubbleController::on_frame_removing(fiorch::WindowFrame* w) {
    if (!w) return;

...
void AiChatScreen::showEvent(QShowEvent* e) {
    ...
    if (link_group_ != fiorch::SymbolGroup::None) {
        const fiorch::SymbolRef rehydrated =
            fiorch::SymbolContext::instance().group_symbol(link_group_);
...
void AiChatScreen::set_group(fiorch::SymbolGroup g) {
    if (link_group_ == g)
        return;
    link_group_ = g;
...
void AiChatScreen::on_group_symbol_changed(const fiorch::SymbolRef& ref) {

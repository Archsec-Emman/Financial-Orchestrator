...
namespace fnt = fiorch::ui::fonts;
namespace col = fiorch::ui::colors;
...
void AiChatScreen::on_stream_chunk(const QString& chunk, bool done) {
    ...
    if (chunk.startsWith("\x01__TOOL_CALL_CLEAR__")) {
        fiorch::ai_chat::ChatBubbleFactory::replace_streaming_text(bubble, "Calling tool...");
        scroll_to_bottom();
        return;
    }

    if (!chunk.isEmpty()) {
        fiorch::ai_chat::ChatBubbleFactory::append_streaming_chunk(bubble, chunk);
        scroll_to_bottom();
    }
    Q_UNUSED(done)
}
...
void AiChatScreen::on_streaming_done(ai_chat::LlmResponse response) {
    ...
    if (!response.success) {
        if (streaming_bubble_) {
            ...
            fiorch::ai_chat::ChatBubbleFactory::replace_streaming_text(streaming_bubble_, err);
        }
    ...
    if (response.content.isEmpty()) {
        if (streaming_bubble_) {
            ...
            fiorch::ai_chat::ChatBubbleFactory::replace_streaming_text(streaming_bubble_, hint);
        }
    ...
    if (streaming_bubble_) {
        ...
        fiorch::ai_chat::ChatBubbleFactory::finalize_streaming(streaming_bubble_, final_text);
        streaming_bubble_ = nullptr;
    }
    if (!content.isEmpty()) {
...
void AiChatScreen::add_message_bubble(const QString& role, const QString& content, const QString& timestamp) {
    fiorch::ai_chat::ChatBubbleFactory::Options opts;
    opts.role               = role;
    ...
    auto b = fiorch::ai_chat::ChatBubbleFactory::build(opts);
...
QLabel* AiChatScreen::add_streaming_bubble() {
    ...
    opts.user_col_max_width = kUserColMaxWidth;
    opts.ai_col_max_width   = kAiColMaxWidth;
    auto b = fiorch::ai_chat::ChatBubbleFactory::build_streaming(opts);
    messages_layout_->insertWidget(messages_layout_->count() - 1, b.row);
    return b.body;
}
...
void AiChatScreen::update_stats() {
    ...
    if (llm.is_configured()) {
        ...
        const bool is_fincept = (provider_raw.toLower() == "fiorch");

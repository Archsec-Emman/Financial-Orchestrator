...
namespace fiorch::screens::equity {
...
QTableWidgetItem* EquityBottomPanel::ensure_item(QTableWidget* table, int row, int col) {
    ...
    if (!item) {
        ...
        item->setForeground(QColor(fiorch::ui::colors::TEXT_PRIMARY()));
...
void EquityBottomPanel::setup_holdings_tab() {
    ...
    strip->setStyleSheet(QString("#eqHoldingsStrip{background:%1;border-bottom:1px solid %2;}")
                             .arg(fiorch::ui::colors::PANEL(), fiorch::ui::colors::BORDER()));
    ...
    auto make_stat = [&](const QString& caption) -> QLabel* {
        ...
        cap->setStyleSheet(QString("color:%1;font-size:10px;letter-spacing:0.5px;")
                               .arg(fiorch::ui::colors::TEXT_SECONDARY()));
        auto* val = new QLabel("--");
        val->setStyleSheet(QString("color:%1;font-size:13px;font-weight:700;")
                               .arg(fiorch::ui::colors::TEXT_PRIMARY()));
        cv->addWidget(cap);
        cv->addWidget(val);
    ...
    holdings_import_btn_->setStyleSheet(
        QString("QPushButton{background:%1;color:%2;border:1px solid %3;padding:4px 12px;"
            ...
            .arg(fiorch::ui::colors::PANEL(), fiorch::ui::colors::ORANGE(),
                 fiorch::ui::colors::BORDER(), fiorch::ui::colors::ORANGE(),
                 fiorch::ui::colors::TEXT_SECONDARY()));
...
void EquityBottomPanel::set_paper_positions(const QVector<trading::PtPosition>& positions) {
    ...
    for (int i = 0; i < positions.size(); ++i) {
        ...
        pnl_item->setForeground(p.unrealized_pnl >= 0 ? QColor(fiorch::ui::colors::POSITIVE())
                                                      : QColor(fiorch::ui::colors::NEGATIVE()));
        ...
        pct_item->setForeground(
            pct >= 0 ? QColor(fiorch::ui::colors::POSITIVE())
                     : QColor(fiorch::ui::colors::NEGATIVE()));
    }
}
...
void EquityBottomPanel::set_paper_trades(const QVector<trading::PtTrade>& trades) {
    ...
    sep_item->setForeground(QColor(fiorch::ui::colors::AMBER()));
    for (int c = 1; c < 8; ++c)
    ...
    for (int i = 0; i < trades.size(); ++i) {
        ...
        pnl_item->setForeground(t.pnl >= 0 ? QColor(fiorch::ui::colors::POSITIVE())
                                           : QColor(fiorch::ui::colors::NEGATIVE()));
...
void EquityBottomPanel::set_paper_stats(const trading::PtStats& stats) {
    ...
    stat_values_[0]->setStyleSheet(
        QString("color: %1;")
            .arg(stats.total_pnl >= 0 ? fiorch::ui::colors::POSITIVE() : fiorch::ui::colors::NEGATIVE()));
    ...
    stat_values_[3]->setStyleSheet(QString("color: %1;").arg(fiorch::ui::colors::POSITIVE()));
    stat_values_[4]->setText(QString::number(stats.largest_loss, 'f', 2));
    stat_values_[4]->setStyleSheet(QString("color: %1;").arg(fiorch::ui::colors::NEGATIVE()));
}
...
void EquityBottomPanel::set_positions(const QVector<trading::BrokerPosition>& positions) {
    ...
    for (int i = 0; i < positions.size(); ++i) {
        ...
        pnl_item->setForeground(p.pnl >= 0 ? QColor(fiorch::ui::colors::POSITIVE())
                                           : QColor(fiorch::ui::colors::NEGATIVE()));

        auto* pct_item = ensure_item(positions_table_, i, 7);
        pct_item->setText(QString("%1%").arg(p.pnl_pct, 0, 'f', 2));
        pct_item->setForeground(p.pnl_pct >= 0 ? QColor(fiorch::ui::colors::POSITIVE())
                                               : QColor(fiorch::ui::colors::NEGATIVE()));
    }
}
...
void EquityBottomPanel::set_holdings(const QVector<trading::BrokerHolding>& holdings) {
    ...
    const QColor pos_color(fiorch::ui::colors::POSITIVE());
    const QColor neg_color(fiorch::ui::colors::NEGATIVE());
    ...
    if (holdings_pnl_label_) {
        ...
        holdings_pnl_label_->setStyleSheet(
            QString("color:%1;font-size:13px;font-weight:700;")
                .arg(total_pnl >= 0 ? fiorch::ui::colors::POSITIVE() : fiorch::ui::colors::NEGATIVE()));
    }
    if (holdings_pnl_pct_label_) {
        ...
        holdings_pnl_pct_label_->setStyleSheet(
            QString("color:%1;font-size:13px;font-weight:700;")
                .arg(total_pct >= 0 ? fiorch::ui::colors::POSITIVE() : fiorch::ui::colors::NEGATIVE()));
    }
    ...
    for (int i = 0; i < orders.size(); ++i) {
        ...
        if (modifiable) {
            ...
            btn->setStyleSheet(QString("QPushButton#eqTableBtn { background: rgba(217,119,6,0.15); "
                                   ...
                                   .arg(fiorch::ui::colors::AMBER())
                                   .arg(fiorch::ui::colors::AMBER_DIM()));
            ...
            connect(btn, &QPushButton::clicked, this, [this, oid, qty, prc]() {
                ...
                dlg->setStyleSheet(QString("QDialog { background: %1; color: %2; }"
                                       ...
                                       .arg(fiorch::ui::colors::BG_SURFACE())
                                       .arg(fiorch::ui::colors::TEXT_PRIMARY())
                                       .arg(fiorch::ui::colors::TEXT_SECONDARY())
                                       .arg(fiorch::ui::colors::BG_BASE())
                                       .arg(fiorch::ui::colors::BORDER_MED())
                                       .arg(fiorch::ui::colors::AMBER()));
                ...
                cancel_btn->setStyleSheet(QString("background: rgba(220,38,38,0.1); color: %1; border: 1px solid %2;")
                                              .arg(fiorch::ui::colors::NEGATIVE())
                                              .arg(fiorch::ui::colors::NEGATIVE_DIM()));
    ...
    for (const auto& auction : auctions) {
        for (const auto& entry : auction.entries) {
            ...
            const QColor type_color =
                is_open ? QColor(fiorch::ui::colors::AMBER()) : QColor(fiorch::ui::colors::INFO());

            auto set = [&](int col, const QString& text,
                           const QColor& fg = QColor(fiorch::ui::colors::TEXT_PRIMARY())) {
            ...
            set(0, auction.date, QColor(fiorch::ui::colors::TEXT_SECONDARY()));
            set(1, is_open ? "OPEN" : "CLOSE", type_color);
            set(2, time_str, QColor(fiorch::ui::colors::TEXT_SECONDARY()));
            set(3, QString::number(entry.price, 'f', 2));
            set(4, QString::number(entry.size, 'f', 0), QColor(fiorch::ui::colors::TEXT_SECONDARY()));
            set(5, entry.exchange, QColor(fiorch::ui::colors::TEXT_SECONDARY()));
        }
    }
...
static void fill_trade_row(QTableWidget* table, int row, const trading::BrokerTrade& t,
    ...
    auto set = [&](int col, const QString& text, const QColor& fg = QColor(fiorch::ui::colors::TEXT_PRIMARY())) {
        auto* item = new QTableWidgetItem(text);
        item->setForeground(fg);
        item->setFlags(Qt::ItemIsEnabled | Qt::ItemIsSelectable);
        table->setItem(row, col, item);
    ...
    set(0, time_str.isEmpty() ? t.timestamp.right(12) : time_str, QColor(fiorch::ui::colors::TEXT_SECONDARY()));
    set(1, QString::number(t.price, 'f', 2),
        t.price > 0 ? QColor(fiorch::ui::colors::TEXT_PRIMARY()) : QColor(fiorch::ui::colors::TEXT_SECONDARY()));
    set(2, QString::number(t.size, 'f', 0), QColor(fiorch::ui::colors::TEXT_SECONDARY()));
    set(3, t.exchange, QColor(fiorch::ui::colors::TEXT_SECONDARY()));
    QStringList decoded;
    for (const auto& c : t.conditions)
        decoded.append(codes.contains(c) ? codes[c] : c);
    set(4, decoded.join(", "), QColor(fiorch::ui::colors::TEXT_TERTIARY()));
    set(5, t.tape, QColor(fiorch::ui::colors::TEXT_TERTIARY()));
}
...
void EquityBottomPanel::setup_calendar_tab() {
    ...
    banner->setStyleSheet(QString("QWidget#calClockBanner { background: %1; border-bottom: 1px solid %2; }")
                              .arg(fiorch::ui::colors::BG_BASE())
                              .arg(fiorch::ui::colors::BORDER_DIM()));
    ...
    clock_status_label_->setStyleSheet(
        QString("color: %1; font-size: 11px; font-weight: 700;").arg(fiorch::ui::colors::TEXT_TERTIARY()));

    clock_next_label_ = new QLabel("");
    clock_next_label_->setObjectName("calClockNext");
    clock_next_label_->setStyleSheet(QString("color: %1; font-size: 10px;").arg(fiorch::ui::colors::TEXT_TERTIARY()));
...
void EquityBottomPanel::set_calendar(const QVector<trading::MarketCalendarDay>& days) {
    ...
    for (int i = 0; i < days.size(); ++i) {
        ...
        auto set = [&](int col, const QString& text) {
            ...
            item->setText(text);
            if (is_today) {
                item->setForeground(QColor(fiorch::ui::colors::AMBER()));
...
void EquityBottomPanel::set_clock(const trading::MarketClock& clock) {
    ...
    if (clock.is_open) {
        ...
        clock_status_label_->setStyleSheet(
            QString("color: %1; font-size: 11px; font-weight: 700;").arg(fiorch::ui::colors::POSITIVE()));
    } else {
        clock_status_label_->setText("● MARKET CLOSED");
        clock_status_label_->setStyleSheet(
            QString("color: %1; font-size: 11px; font-weight: 700;").arg(fiorch::ui::colors::NEGATIVE()));
    }
    ...
    if (!clock.next_open.isEmpty() || !clock.next_close.isEmpty()) {
        ...
        clock_next_label_->setStyleSheet(
            QString("color: %1; font-size: 10px;").arg(fiorch::ui::colors::TEXT_SECONDARY()));
    }
}

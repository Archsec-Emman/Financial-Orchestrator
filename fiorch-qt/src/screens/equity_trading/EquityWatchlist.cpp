        ...
        for (const auto& e : entries_) {
            if (e.symbol == sym && e.has_data) {
                ...
                if (chg_item) {
                    ...
                    chg_item->setForeground(e.change_pct >= 0 ? QColor(fiorch::ui::colors::POSITIVE())
...
void EquityWatchlist::set_active_symbol(const QString& symbol) {
    ...
    for (int row = 0; row < table_->rowCount(); ++row) {
        ...
        for (int col = 0; col < 3; ++col) {
            ...
            if (cell) {
                cell->setBackground(active ? QColor(fiorch::ui::colors::BG_HOVER())
                                           : QColor(fiorch::ui::colors::BG_BASE()));
            }
        }
    }
}
...
void EquityWatchlist::rebuild_table() {
    ...
    for (int i = 0; i < entries_.size(); ++i) {
        ...
        sym_item->setForeground(QColor(fiorch::ui::colors::TEXT_PRIMARY()));
        ...
        ltp_item->setForeground(QColor(fiorch::ui::colors::TEXT_PRIMARY()));
...
void EquityWatchlist::on_add_symbol_entered() {
    ...
    if (!broker_id_.isEmpty() && trading::InstrumentService::instance().is_loaded(broker_id_)) {
        ...
        if (results.isEmpty()) {
            ...
            add_edit_->setStyleSheet(QString("border: 1px solid %1;").arg(fiorch::ui::colors::NEGATIVE()));

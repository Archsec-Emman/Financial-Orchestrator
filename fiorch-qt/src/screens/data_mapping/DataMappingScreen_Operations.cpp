...
using namespace fiorch::screens::data_mapping_internal;
...
void DataMappingScreen::on_test_api() {
    ...
    using fiorch::services::DataMappingTestClient;
...
void DataMappingScreen::on_run_mapping() {
    ...
    fiorch::services::DataNormalizationService::instance().fetch_and_normalize(
        dm, [self, dm](bool ok, fiorch::services::NormalizedRecord rec) {
            if (!self)
                return;
            if (ok) {

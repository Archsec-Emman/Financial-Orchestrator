...
namespace fiorch::screens::alpha_arena {

using fiorch::services::alpha_arena::AlphaArenaEngine;
using fiorch::services::alpha_arena::AlphaArenaRepo;
using fiorch::services::alpha_arena::AgentRow;
using fiorch::services::alpha_arena::EventRow;
using fiorch::services::alpha_arena::Position;
...
void PositionsPanel::refresh() {
    ...
    for (int i = 0; i < rows.size(); ++i) {
        ...
        it_qty->setForeground(QColor(p.qty >= 0 ? fiorch::ui::colors::POSITIVE()
        ...
        it_pnl->setForeground(QColor(p.unrealized_pnl >= 0 ? fiorch::ui::colors::POSITIVE()
                                                            : fiorch::ui::colors::NEGATIVE()));
        ...
        if (dist < 0.05) it_dist->setForeground(QColor(fiorch::ui::colors::NEGATIVE()));
...
void RiskPanel::refresh() {
    ...
    for (int i = 0; i < agents.value().size(); ++i) {
        ...
        if (a.state == QLatin1String("circuit_open") || a.state == QLatin1String("halted"))
            it_state->setForeground(QColor(fiorch::ui::colors::NEGATIVE()));
        ...
        it_dd->setForeground(QColor(fiorch::ui::colors::NEGATIVE()));
...
void AuditPanel::refresh() {
    ...
    for (const auto& e : rows.value()) {
        ...
        if (e.type.contains(QLatin1String("error")) ||
            ...
            it->setForeground(QColor(fiorch::ui::colors::NEGATIVE()));
        } else if (e.type.contains(QLatin1String("decision")) ||
                   e.type.contains(QLatin1String("fill"))) {
            it->setForeground(QColor(fiorch::ui::colors::POSITIVE()));
        }
        list_->addItem(it);

...
void LaunchpadScreen::on_open_layout() {
    ...
    QMetaObject::invokeMethod(this, [ws]() {
        if (fiorch::WindowRegistry::instance().frames().isEmpty()) {
            auto* fr = new fiorch::WindowFrame(fiorch::WindowFrame::next_window_id());
...
void LaunchpadScreen::on_continue() {
    ...
    QMetaObject::invokeMethod(this, []() {
        ...
        if (n == 0) {
            ...
            auto* w = new fiorch::WindowFrame(fiorch::WindowFrame::next_window_id());
...
void LaunchpadScreen::on_template_picked(const QString& persona_id) {
    ...
    QMetaObject::invokeMethod(this, [ws, first_run_tour]() {
        fiorch::WindowFrame* anchor = nullptr;
        if (fiorch::WindowRegistry::instance().frames().isEmpty()) {
            anchor = new fiorch::WindowFrame(fiorch::WindowFrame::next_window_id());
            anchor->setAttribute(Qt::WA_DeleteOnClose);
            anchor->show();
        } else {
            anchor = fiorch::WindowRegistry::instance().frames().first();
        }
        layout::WorkspaceShell::apply(ws);
        ...
        if (first_run_tour && anchor) {
            QPointer<fiorch::WindowFrame> guard(anchor);

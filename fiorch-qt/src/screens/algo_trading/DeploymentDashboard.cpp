...
inline QString kMonoFont() {
    return QString("font-family: %1;").arg(fiorch::ui::fonts::DATA_FAMILY);
}
...
using namespace fiorch::services::algo;
...
static QWidget* build_stat_card(const QString& label, const QString& value, const QString& color, QLabel** out_label,
    ...
    card->setStyleSheet(QString("background: %1; border: 1px solid %2;")
                            .arg(fiorch::ui::colors::BG_SURFACE(), fiorch::ui::colors::BORDER_DIM()));
    ...
    lbl->setStyleSheet(QString("color: %1; font-size: %2px; font-weight: 700; letter-spacing: 0.5px; %3"
                               " background: transparent; border: none;")
                           .arg(fiorch::ui::colors::TEXT_TERTIARY())
                           .arg(fiorch::ui::fonts::TINY)
                           .arg(kMonoFont()));
    vl->addWidget(lbl);
...
QWidget* DeploymentDashboard::build_deployment_card(const AlgoDeployment& d, QWidget* parent) {
    ...
    mode_badge->setStyleSheet(QString("color: %1; font-size: %2px; font-weight: 700; %3"
                                  ...
                                  .arg(is_paper ? fiorch::ui::colors::POSITIVE() : fiorch::ui::colors::AMBER())
                                  .arg(fiorch::ui::fonts::TINY)
                                  .arg(kMonoFont())
    ...
    auto add_metric = [&](const QString& label, const QString& value, const QString& color,
                          int font_size = fiorch::ui::fonts::SMALL) {
    ...
    add_metric("P&L", QString("%1$%2").arg(total_pnl >= 0 ? "+" : "-").arg(std::abs(total_pnl), 0, 'f', 2),
               total_pnl >= 0 ? fiorch::ui::colors::POSITIVE : fiorch::ui::colors::NEGATIVE,
               fiorch::ui::fonts::TITLE);

    // Win Rate
    add_metric("WIN RATE", QString("%1%").arg(d.win_rate, 0, 'f', 1), fiorch::ui::colors::TEXT_PRIMARY);
    ...
    if (d.position_qty != 0) {
        add_metric("POSITION",
                   ...
                   fiorch::ui::colors::TEXT_PRIMARY);
    } else {
        add_metric("POSITION", "FLAT", fiorch::ui::colors::TEXT_TERTIARY);
    }

    // Trades
    add_metric("TRADES", QString::number(d.total_trades), fiorch::ui::colors::TEXT_PRIMARY);

    // Max Drawdown
    add_metric("MAX DD", QString("-%1%").arg(std::abs(d.max_drawdown), 0, 'f', 2), fiorch::ui::colors::NEGATIVE);
    ...
    win_bar->setStyleSheet(
        QString("QProgressBar { background: %1; border: none; border-radius: 2px; }"
            ...
            .arg(fiorch::ui::colors::BG_RAISED(), fiorch::ui::colors::POSITIVE()));
...
void DeploymentDashboard::update_summary(const QVector<AlgoDeployment>& deployments) {
    ...
    if (total_pnl_) {
        ...
        total_pnl_->setStyleSheet(QString("color: %1; font-size: %2px; font-weight: 700; %3"
                                          " background: transparent; border: none;")
                                      .arg(pnl_sum >= 0 ? fiorch::ui::colors::POSITIVE() : fiorch::ui::colors::NEGATIVE())
                                      .arg(fiorch::ui::fonts::TITLE)
                                      .arg(kMonoFont()));
    }
...
void DeploymentDashboard::build_ui() {
    ...
    scroll->setStyleSheet(QString("QScrollArea { background: %1; border: none; }"
                              ...
                              .arg(fiorch::ui::colors::BG_BASE(), fiorch::ui::colors::BORDER_MED()));

    auto* content = new QWidget(this);
    content->setStyleSheet(QString("background: %1;").arg(fiorch::ui::colors::BG_BASE()));
    ...
    summary_row->addWidget(
        build_stat_card("ACTIVE DEPLOYMENTS", "0", fiorch::ui::colors::CYAN, &active_count_, content));
    summary_row->addWidget(build_stat_card("TOTAL P&L", "$0.00", fiorch::ui::colors::POSITIVE, &total_pnl_, content));
    summary_row->addWidget(
        build_stat_card("TOTAL TRADES", "0", fiorch::ui::colors::TEXT_PRIMARY, &total_trades_, content));
    summary_row->addWidget(
        build_stat_card("AVG WIN RATE", "0.0%", fiorch::ui::colors::TEXT_PRIMARY, &avg_win_rate_, content));
    ...
    refresh_btn->setStyleSheet(
        QString("QPushButton { background: %1; color: %2; border: 1px solid %3;"
            ...
            .arg(fiorch::ui::colors::BG_RAISED(), fiorch::ui::colors::TEXT_SECONDARY(), fiorch::ui::colors::BORDER_DIM())
            .arg(fiorch::ui::fonts::TINY)
            .arg(kMonoFont())
            .arg(fiorch::ui::colors::BG_HOVER(), fiorch::ui::colors::TEXT_PRIMARY()));

...
static const QString kIn =
    QString("background:%1;color:%2;border:1px solid %3;padding:4px 8px;font-size:12px;")
        .arg(fiorch::ui::colors::BG_RAISED(), fiorch::ui::colors::TEXT_PRIMARY(), fiorch::ui::colors::BORDER_MED());
} // namespace
...
QWidget* PlannerViewPanel::build_templates_panel() {
    ...
    {
        const auto pr = fiorch::LlmProfileRepository::instance().list_profiles();
        const auto profiles = pr.is_ok() ? pr.value() : QVector<fiorch::LlmProfile>{};
    ...
    {
        auto& repo = fiorch::PortfolioRepository::instance();

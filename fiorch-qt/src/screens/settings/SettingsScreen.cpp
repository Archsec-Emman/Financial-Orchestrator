...
void SettingsScreen::wire_section_signals() {
    ...
    if (auto* voice = qobject_cast<VoiceConfigSection*>(sections_->widget(13))) {
        connect(voice, &VoiceConfigSection::config_changed, this, []() {
            fiorch::services::SpeechService::instance().reload_config();
            fiorch::services::TtsService::instance().reload_config();

            auto& clap = fiorch::services::ClapDetectorService::instance();
            clap.stop();
            if (fiorch::services::ClapDetectorService::is_enabled_in_config())
                clap.start();
        });
    }
}

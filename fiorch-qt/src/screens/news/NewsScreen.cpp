...
void NewsScreen::connect_signals() {
    ...
    connect(detail_panel_, &NewsDetailPanel::bookmark_requested, this, [this](const services::NewsArticle& article) {
        auto r = fiorch::NewsArticleRepository::instance().toggle_saved(article.id);
        if (r.is_ok()) {
            auto saved_r = fiorch::NewsArticleRepository::instance().load_saved();
            if (saved_r.is_ok())
    ...
    connect(seen_flush_timer_, &QTimer::timeout, this, [this]() {
        ...
        (void)QtConcurrent::run([ids, self]() {
            for (const auto& id : ids)
                fiorch::NewsArticleRepository::instance().mark_seen(id);
            if (self) {
...
void NewsScreen::showEvent(QShowEvent* e) {
    ...
    if (first_show_) {
        ...
        fiorch::NewsArticleRepository::instance().ensure_seen_column();
        fiorch::NewsArticleRepository::instance().ensure_saved_column();

        int64_t since_ts = QDateTime::currentSecsSinceEpoch() - (30LL * 86400);
        auto seen_result = fiorch::NewsArticleRepository::instance().load_seen_ids(since_ts);
        if (seen_result.is_ok()) {
            for (const auto& id : seen_result.value())
                feed_panel_->model()->mark_seen(id);
        }

        auto saved_result = fiorch::NewsArticleRepository::instance().load_saved();
...
void NewsScreen::apply_filters_async() {
    ...
    if (time_range == "7D" || time_range == "30D") {
        ...
        if (!search_lower.isEmpty()) {
            auto fts_result = fiorch::NewsArticleRepository::instance().search_fts(search_lower, cutoff, 1000);
            if (fts_result.is_ok())
                merge_into(fts_result.value());
        } else {
            auto db_result = fiorch::NewsArticleRepository::instance().load_recent(cutoff, {}, 5000);
            if (db_result.is_ok())
...
void NewsScreen::update_ui_from_filtered(int /*generation*/, const QVector<services::NewsArticle>& filtered,
    ...
    if (!breaking.isEmpty()) {
        ...
        if (get_bool("notifications.news_breaking", true)) {
            using namespace fiorch::notifications;

...
DashboardScreen::DashboardScreen(QWidget* parent) : QWidget(parent) {
    ...
    notif_toast_ = new fiorch::ui::NotifToast(this);
    connect(&fiorch::notifications::NotificationService::instance(),
            &fiorch::notifications::NotificationService::notification_received, notif_toast_,
            &fiorch::ui::NotifToast::show_notification);
    ...
    connect(toolbar_, &DashboardToolBar::reset_layout_clicked, this, [this]() {
        ...
        connect(dlg, &TemplatePicker::template_selected, this, [this](const QString& tid) {
            ...
            (void)QtConcurrent::run([]() { fiorch::SettingsRepository::instance().remove("dashboard_canvas_layout"); });
...
void DashboardScreen::save_layout() {
    ...
    (void)QtConcurrent::run(
        [encoded]() { fiorch::SettingsRepository::instance().set("dashboard_canvas_layout", encoded, "dashboard"); });
}

void DashboardScreen::restore_layout() {
    QPointer<DashboardScreen> self = this;
    (void)QtConcurrent::run([self]() {
        auto result = fiorch::SettingsRepository::instance().get("dashboard_canvas_layout");

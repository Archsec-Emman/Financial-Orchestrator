...
void CryptoTickerWidget::hub_resubscribe() {
    ...
    for (const auto& pair : pairs_) {
        ...
        hub.subscribe(this, topic, [this, pair_copy](const QVariant& v) {
            if (!v.canConvert<fiorch::trading::TickerData>())
                return;
            on_ticker(pair_copy, v.value<fiorch::trading::TickerData>());
        });
    }
    hub_active_ = true;
}
...
void CryptoTickerWidget::on_ticker(const QString& pair, const fiorch::trading::TickerData& t) {

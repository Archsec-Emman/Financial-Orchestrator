...
namespace fiorch::screens::widgets {
...
GeopoliticsEventsWidget::GeopoliticsEventsWidget(const QJsonObject& cfg, QWidget* parent)
    : BaseWidget(tr("GEOPOLITICS EVENTS"), parent, ui::colors::NEGATIVE()) {
    ...
    make_hdr(tr("CAT"), 1);
    make_hdr(tr("EVENT"), 5);
    make_hdr(tr("LOCATION"), 2);
    make_hdr(tr("SRC"), 1);
    make_hdr(tr("AGE"), 1, Qt::AlignRight);
...
void GeopoliticsEventsWidget::populate(const QVariant& payload) {
    ...
    using fiorch::services::geo::EventsPage;
    ...
    for (const auto& ev : page.events) {
        ...
        const QColor cat_col = fiorch::services::geo::category_color(ev.event_category);
        const QString cat_label = fiorch::services::geo::pretty_category(ev.event_category);

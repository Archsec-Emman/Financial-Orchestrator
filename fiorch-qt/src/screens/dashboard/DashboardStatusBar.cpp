...
DashboardStatusBar::DashboardStatusBar(QWidget* parent) : QWidget(parent) {
    ...
    // ── Notification bell ──────────────────────────────────────────────────
    notif_bell_ = new fiorch::ui::NotifBell(this);
    rl->addWidget(notif_bell_);

    connect(notif_bell_, &fiorch::ui::NotifBell::bell_clicked, this, &DashboardStatusBar::toggle_notif_panel);
...
void DashboardStatusBar::ping_api() {
    QNetworkRequest req(QUrl("https://api.fiorch.in/health"));
...
void DashboardStatusBar::toggle_notif_panel() {
    ...
    if (!notif_panel_) {
        QWidget* root = window();
        notif_panel_ = new fiorch::ui::NotifPanel(root ? root : this);
    }

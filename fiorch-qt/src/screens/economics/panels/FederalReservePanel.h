// src/screens/economics/panels/FederalReservePanel.h
// US Federal Reserve data panel.
// Script: federal_reserve_data.py  |  No API key required.
#pragma once

#include "screens/economics/panels/EconPanelBase.h"

#include <QComboBox>

namespace fiorch::screens {

class FederalReservePanel : public EconPanelBase {
    Q_OBJECT
  public:
    explicit FederalReservePanel(QWidget* parent = nullptr);
    void activate() override;

  protected:
    void build_controls(QHBoxLayout* thl) override;
    void on_fetch() override;
    void on_result(const QString& request_id, const services::EconomicsResult& result) override;

  private:
    QComboBox* series_combo_ = nullptr;
};

} // namespace fiorch::screens

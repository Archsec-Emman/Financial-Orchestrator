...
WtoPanel::WtoPanel(QWidget* parent) : EconPanelBase(kWtoSourceId, kWtoColor, parent) {
    build_base_ui(this);
...
void WtoPanel::on_result(const QString& request_id, const services::EconomicsResult& result) {
    if (result.source_id != kWtoSourceId)
        return;

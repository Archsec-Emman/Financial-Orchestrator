...
class LegEditorModel : public QAbstractTableModel {
  ...
  public:
    ...
    // ── Mutation API ────────────────────────────────────────────────────────
    void set_legs(const QVector<fiorch::services::options::StrategyLeg>& legs);
    const QVector<fiorch::services::options::StrategyLeg>& legs() const { return legs_; }
    /// Append a single leg (used by the chain "leg_clicked" hookup).
    void append_leg(const fiorch::services::options::StrategyLeg& leg);
  ...
  private:
    QVector<fiorch::services::options::StrategyLeg> legs_;
};

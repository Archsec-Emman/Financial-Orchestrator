...
namespace fiorch::screens::fno {

using fiorch::services::options::OptionChain;
using fiorch::services::options::Strategy;
using fiorch::services::options::StrategyAnalytics;
using fiorch::services::options::StrategyInstantiationOptions;
using fiorch::services::options::analytics::PayoffComputeOptions;
...
void BuilderSubTab::hideEvent(QHideEvent* e) {
    ...
    if (!chain_subscribed_)
        return;
    fiorch::datahub::DataHub::instance().unsubscribe_pattern(this, QStringLiteral("option:chain:*"));
...
void BuilderSubTab::on_template_chosen(const QString& template_id, const StrategyInstantiationOptions& options) {
    ...
    const auto* tpl = fiorch::services::options::find(template_id);
    if (!tpl) {
        LOG_WARN("FnoBuilder", QString("Unknown template id: %1").arg(template_id));
        return;
    }
    auto r = fiorch::services::options::instantiate(*tpl, last_chain_, options);
    if (r.is_err()) {
...
void BuilderSubTab::refresh_analytics() {
    ...
    auto curve = fiorch::services::options::analytics::compute_payoff(s, opts);
    auto bes = fiorch::services::options::analytics::compute_breakevens(curve);
    auto a = fiorch::services::options::analytics::compute_all(s, last_chain_, opts);
...
QString BuilderSubTab::ensure_paper_portfolio() {
    ...
    auto existing = fiorch::trading::pt_find_portfolio(name, exchange);
    ...
    auto p = fiorch::trading::pt_create_portfolio(name, /*balance*/ 1'000'000.0,
                                                   /*currency*/ "INR",
                                                   /*leverage*/ 1.0,
                                                   /*margin_mode*/ "cross",
                                                   /*fee_rate*/ 0.0005,
...
void BuilderSubTab::on_trade_clicked() {
    ...
    fiorch::services::options::analytics::PayoffComputeOptions opts;
    opts.current_spot = last_chain_.spot;
    auto a = fiorch::services::options::analytics::compute_all(s, last_chain_, opts);
    ...
    for (const auto& leg : legs) {
        ...
        try {
            fiorch::trading::pt_place_order(portfolio_id, leg.symbol, side, "limit", qty,
                                             leg.entry_price);
            ++placed;
...
void BuilderSubTab::on_save_clicked() {
    ...
    // name and INSERT.
    auto& repo = fiorch::StrategiesRepository::instance();
...
void BuilderSubTab::on_load_clicked() {
    ...
    auto rows_r = fiorch::StrategiesRepository::instance().list_all();
    if (rows_r.is_err() || rows_r.value().isEmpty()) {
        ...
        for (const auto& row : rows_r.value()) {
            ...
            connect(act, &QAction::triggered, this, [this, id, name_for_msg]() {
                auto r = fiorch::StrategiesRepository::instance().get(id);
                if (r.is_err()) {
        ...
        for (const auto& row : rows_r.value()) {
            ...
            connect(del, &QAction::triggered, this, [this, id, name]() {
                ...
                if (choice != QMessageBox::Yes)
                    return;
                fiorch::StrategiesRepository::instance().remove(id);
            });
        }
    }

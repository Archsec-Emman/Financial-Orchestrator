...
using fiorch::services::options::StrategyCategory;
using fiorch::services::options::StrategyInstantiationOptions;
using fiorch::services::options::StrategyTemplate;
using fiorch::services::options::catalog;
using fiorch::services::options::find;
...
void TemplatePickerPanel::on_template_selected() {
    ...
    // Fully qualify — `find` is a free function in fiorch::services::options,
    // but inside a QWidget subclass the unqualified name resolves to
    // QWidget::find(WId) first.
    selected_ = fiorch::services::options::find(id);
    if (selected_)

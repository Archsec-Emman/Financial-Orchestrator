...
class OrderConfirmDialog : public QDialog {
    Q_OBJECT
  public:
    OrderConfirmDialog(const fiorch::services::options::Strategy& strategy,
                       const fiorch::services::options::OptionChain& chain,
    ...
    std::optional<fiorch::trading::BasketMargin> basket_margin() const { return margin_; }

    const fiorch::services::options::Strategy& strategy() const { return strategy_; }

  private:
    void setup_ui(double premium, double max_profit, double max_loss);
    void populate_legs();
    void start_margin_fetch();
    void on_margin_loaded(bool ok, fiorch::trading::BasketMargin margin, QString error);

    fiorch::services::options::Strategy strategy_;
    fiorch::services::options::OptionChain chain_;
    std::optional<fiorch::trading::BasketMargin> margin_;

...
using fiorch::services::options::OptionChainService;
using fiorch::trading::AccountManager;
using fiorch::trading::BrokerAccount;
...
void ChainSubTab::hideEvent(QHideEvent* e) {
    ...
    if (!active_topic_.isEmpty()) {
        fiorch::datahub::DataHub::instance().unsubscribe(this, active_topic_);
        active_topic_.clear();
    }
}
...
void ChainSubTab::on_underlying_changed(const QString& underlying) {
    ...
    if (underlying.isEmpty())
        return;
    fiorch::SymbolGroup link_group = fiorch::SymbolGroup::None;
    for (QObject* p = parent(); p; p = p->parent()) {
        if (auto* gl = dynamic_cast<fiorch::IGroupLinked*>(p)) {
            link_group = gl->group();
            break;
        }
    }
    if (link_group == fiorch::SymbolGroup::None)
        return;
    ...
    fiorch::SymbolContext::instance().set_group_symbol(link_group, ref, this);
}
...
void ChainSubTab::on_refresh_clicked() {
    ...
    if (topic.isEmpty())
        return;
    fiorch::datahub::DataHub::instance().request(topic, /*force*/ true);
}

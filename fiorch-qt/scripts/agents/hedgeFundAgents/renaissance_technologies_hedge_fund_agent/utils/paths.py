"""
Cross-Platform Path Utilities for Financial Orchestrator
...
- Windows: %APPDATA%\fiorch-terminal\
- macOS: ~/Library/Application Support/fiorch-terminal/
- Linux: ~/.local/share/fiorch-terminal/
"""
...
def get_app_data_dir() -> Path:
    """
    Get the Financial Orchestrator application data directory.
    ...
    Returns:
        Path to fiorch-terminal app data directory
    """
...
def get_rentech_data_dir() -> Path:
    ...
    Returns:
        ...
        (e.g., %APPDATA%\fiorch-terminal\rentech\)
    """
...
def get_knowledge_base_path() -> Path:
    ...
    Returns:
        ...
        (e.g., %APPDATA%\fiorch-terminal\rentech\knowledge\)
    """
...
def get_memory_db_path() -> Path:
    ...
    Returns:
        ...
        (e.g., %APPDATA%\fiorch-terminal\rentech\memory.db)
    """
...
def get_logs_dir() -> Path:
    ...
    Returns:
        ...
        (e.g., %APPDATA%\fiorch-terminal\rentech\logs\)
    """
...
def get_cache_dir() -> Path:
    ...
    Returns:
        ...
        (e.g., %APPDATA%\fiorch-terminal\rentech\cache\)
    """
...
def get_frontend_db_path() -> Optional[Path]:
    """
    Get the path to frontend SQLite database (fiorch.db).

    Searches common locations for the frontend database.

    Returns:
        Path to fiorch.db if found, None otherwise
    """
    ...
    db_path = app_data / "fiorch.db"
    if db_path.exists():
        return db_path

    # Alternative locations
    alternatives = [
        app_data.parent / "fiorch.db",  # One level up
        Path.home() / ".fiorch" / "fiorch.db",
        Path("./fiorch.db"),  # Current directory (dev mode)
    ]
    ...
    logger.warning("Frontend database (fiorch.db) not found")
    return None
...
def print_paths():
    ...
    print("=== Financial Orchestrator - RenTech Agent Paths ===")

...
def base_dir() -> str:
    ...
    <LOCALAPPDATA>/com.fiorch.terminal/finagent on Windows, or
    $XDG_DATA_HOME/com.fiorch.terminal/finagent (~/.local/share fallback)
    elsewhere.
    """
    ...
    if os.name == "nt":
        ...
        return str(Path(root) / "com.fiorch.terminal" / "finagent")
    xdg = os.getenv("XDG_DATA_HOME") or str(Path.home() / ".local" / "share")
    return str(Path(xdg) / "com.fiorch.terminal" / "finagent")

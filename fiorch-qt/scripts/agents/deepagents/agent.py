"""
agent.py — Full create_deep_agent wiring for Financial Orchestrator.
...
_FINCEPT_SYSTEM_PROMPT = """You are a Deep Agent for Financial Orchestrator, an institutional-grade financial intelligence platform.
...
def create_agent(
    ...
    compiled = create_deep_agent(
        ...
        debug=debug,
        name=f"fiorch-{agent_type}-agent",
        cache=_cache,
    )

    logger.info(
        "Created fiorch-%s-agent with %d subagents, backend=%s",

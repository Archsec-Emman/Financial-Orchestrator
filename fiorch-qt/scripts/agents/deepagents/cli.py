...
def cmd_stream_task(params: dict[str, Any]) -> None:
    ...
    try:
        from agent import create_agent, build_fiorch_context
        from models import create_model

        model    = create_model(config)
        compiled = create_agent(model=model, config={**config, "agent_type": agent_type})
        ctx      = build_fiorch_context({**params, "agent_type": agent_type})
...
def cmd_resume_thread(params: dict[str, Any]) -> dict[str, Any]:
    ...
    try:
        ...
        ctx      = build_fiorch_context(params)
...
def _execute_with_deepagents(
    ...
    try:
        ...
        ctx = build_fiorch_context({"agent_type": agent_type, "context": context_data})

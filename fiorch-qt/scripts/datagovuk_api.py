#!/usr/bin/env python3
"""
data.gov.uk (CKAN) API Wrapper
Fetches UK government data using hierarchical structure:
...
Usage: python datagovuk_api.py <command> [args]
"""
...
API_KEY = os.environ.get('DATAGOVUK_API_KEY')
TIMEOUT = 30

def _make_request(action: str, params: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
    """
    Centralized request handler for data.gov.uk API
    ...
    try:
        ...
        if not data.get('success', False):
            ...
            return {
                "data": [],
                "metadata": {"source": "data.gov.uk", "last_updated": datetime.now().isoformat()},
        ...
        return {
            "data": [],
            "metadata": {},
            "error": "Request timeout. The data.gov.uk API is taking too long to respond."
        }
    except requests.exceptions.ConnectionError:
        return {
            "data": [],
            "metadata": {},
            "error": "Connection error. Unable to connect to data.gov.uk API."
        }
        ...
        return {
            "data": [],
            "metadata": {},
            "error": "Invalid JSON response from data.gov.uk API"
        }
...
def download_resource_preview(resource_url: str, max_lines: int = 10) -> Dict[str, Any]:
    ...
    try:
        ...
        if not ('csv' in content_type or 'text' in content_type or 'excel' in content_type):
            return {
                "data": [],
...
def main():
    ...
    if len(sys.argv) < 2:
        print(json.dumps({
            "error": "Usage: python datagovuk_api.py <command> <args>",
            "available_commands": [
                "publishers",

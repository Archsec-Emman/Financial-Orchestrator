#!/usr/bin/env python3
"""
Executes strategies from the 418+ Financial Orchestrator strategy registry.
...
def _enrich_metrics_from_equity(
    ...
    """
    Bridges Financial Orchestrator frontend to 418+ QCAlgorithm strategies.
    """
    ...
    def get_strategies(self, request: Dict[str, Any]) -> Dict[str, Any]:
        ...
        return {'success': True, 'data': {'provider': 'fiorch', 'strategies': grouped}}
...
def main():
    ...
    if len(sys.argv) < 3:
        print(json_response({
            'success': False,
            'error': 'Usage: fiorch_provider.py <command> <args_json>',
        }))
        sys.exit(1)
    ...
    if command in ('run_backtest', 'execute_fiorch_strategy'):
